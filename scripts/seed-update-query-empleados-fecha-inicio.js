/**
 * Inserta ALTER Empleados.FECHA_INICIO en UPDATE_QUERIES (hosting).
 * Uso: node scripts/seed-update-query-empleados-fecha-inicio.js
 */
...
async function main() {
  ...
  let qry = fs
    .readFileSync(path.join(__dirname, 'sql', 'dbo.Empleados.FECHA_INICIO.sql'), 'utf8')
    .replace(/^\uFEFF/, '')
    .replace(/\r\n/g, '\n')
    .trim();
  qry = qry
  ...
  const check = await pool
    ...
    .query(`
      ...
      WHERE VERSION = @VERSION AND DB = @DB AND QRY LIKE '%FECHA_INICIO%' AND QRY LIKE '%Empleados%'
    `);
