...
async function main() {
  ...
  const cols = await pool.request().query(`
    ...
    WHERE TABLE_NAME = 'VEHICULOS_KILOMETRAJES'
    ORDER BY ORDINAL_POSITION
  `);
  console.log('VEHICULOS_KILOMETRAJES');
  console.log(JSON.stringify(cols.recordset, null, 2));
  try {
    const sample = await pool.request().query('SELECT TOP 3 * FROM dbo.VEHICULOS_KILOMETRAJES');
