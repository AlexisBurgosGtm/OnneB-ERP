/**
 * Inserta CREATE TABLE DOCUMENTOS_ELIMINADOS en UPDATE_QUERIES (hosting).
 * Uso: node scripts/seed-update-query-documentos-eliminados.js
 */
...
async function main() {
  ...
  let qry = fs
    .readFileSync(path.join(__dirname, 'sql', 'dbo.DOCUMENTOS_ELIMINADOS.sql'), 'utf8')
    .replace(/^\uFEFF/, '')
    .replace(/\r\n/g, '\n')
    .trim();
  qry = qry
  ...
  const check = await pool
    ...
    .query(`
      ...
      WHERE VERSION = @VERSION AND DB = @DB AND QRY LIKE '%DOCUMENTOS_ELIMINADOS%'
    `);
