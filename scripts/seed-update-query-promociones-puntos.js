/**
 * PROMOCIONES: FACTOR_PUNTOS, TIPO, CODCLAUNO → UPDATE_QUERIES.
 * Uso (OnneB o FS-SV; misma BD UPDATE_* compartida):
 *   node scripts/seed-update-query-promociones-puntos.js
 */
...
async function main() {
  ...
  const chunks = readSqlChunksFromScripts('dbo.PROMOCIONES_PUNTOS_COLUMNS.sql');
  const pool = await getUpdateDbPool();

  for (const chunk of chunks) {
    const marker = `PROMOCIONES_PUNTOS:${chunk.name}`;
