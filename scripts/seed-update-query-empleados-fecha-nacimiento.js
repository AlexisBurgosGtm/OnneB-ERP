/**
 * Inserta ALTER Empleados.FECHA_NACIMIENTO en UPDATE_QUERIES (hosting).
 * Uso: node scripts/seed-update-query-empleados-fecha-nacimiento.js
 */
...
async function main() {
  ...
  let qry = fs
    .readFileSync(path.join(__dirname, 'sql', 'dbo.Empleados.FECHA_NACIMIENTO.sql'), 'utf8')
    .replace(/^\uFEFF/, '')
    .replace(/\r\n/g, '\n')
    .trim();
  qry = qry
  ...
  const check = await pool
    ...
    .query(`
      ...
      WHERE VERSION = @VERSION AND DB = @DB AND QRY LIKE '%FECHA_NACIMIENTO%' AND QRY LIKE '%Empleados%'
    `);
