/**
 * PROMOCIONES.VALORPUNTO → UPDATE_QUERIES.
 * Uso (OnneB o FS-SV; misma BD UPDATE_* compartida):
 *   node scripts/seed-update-query-promociones-valorpunto.js
 */
...
async function main() {
  ...
  const chunks = readSqlChunksFromScripts('dbo.PROMOCIONES_VALORPUNTO.sql');
  const pool = await getUpdateDbPool();

  for (const chunk of chunks) {
    const marker = `PROMOCIONES_VALORPUNTO:${chunk.name}`;
