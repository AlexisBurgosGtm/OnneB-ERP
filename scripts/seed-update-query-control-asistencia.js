/**
 * Inserta CREATE TABLE CONTROL_ASISTENCIA en UPDATE_QUERIES (hosting).
 * Uso: node scripts/seed-update-query-control-asistencia.js
 */
...
async function main() {
  ...
  let qry = fs
    .readFileSync(path.join(__dirname, 'sql', 'dbo.CONTROL_ASISTENCIA.sql'), 'utf8')
    .replace(/^\uFEFF/, '')
    .replace(/\r\n/g, '\n')
    .trim();
  qry = qry
  ...
  const check = await pool
    ...
    .query(`
      ...
      WHERE VERSION = @VERSION AND DB = @DB
        AND QRY LIKE '%CONTROL_ASISTENCIA%'
