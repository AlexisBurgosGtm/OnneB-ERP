/**
 * Inserta ALTER NOMINA_EMPLEADO / NOMINA_DETALLE (Departamento, BonoLey, BonoAdicional)
 * en UPDATE_QUERIES (hosting).
 * Uso: node scripts/seed-update-query-nomina-bonos-depto.js
 */
...
async function main() {
  ...
  let qry = fs
    .readFileSync(path.join(__dirname, 'sql', 'dbo.NOMINA_EMPLEADO_BONOS_DEPTO.sql'), 'utf8')
    .replace(/^\uFEFF/, '')
    .replace(/\r\n/g, '\n')
    .trim();
  qry = qry
