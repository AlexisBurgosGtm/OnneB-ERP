/**
 * PROMOCIONES_PAGOS → UPDATE_QUERIES.
 * Uso (OnneB o FS-SV; misma BD UPDATE_* compartida):
 *   node scripts/seed-update-query-promociones-pagos.js
 */
...
async function main() {
  ...
  const chunks = readSqlChunksFromScripts('dbo.PROMOCIONES_PAGOS.sql');
  const pool = await getUpdateDbPool();

  for (const chunk of chunks) {
    const marker = `PROMOCIONES_PAGOS:${chunk.name}`;
