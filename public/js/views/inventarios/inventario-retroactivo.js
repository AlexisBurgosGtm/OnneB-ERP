/**
 * Vista Inventario Retroactivo — mismo layout que Inventario,
 * saldo recalculado por movimientos hasta Mes/Año.
 */
const INV_RETRO_MESES = [
  { value: 1, label: 'Enero' },
...
const INV_RETRO_ANIOS = [];
(() => {
  const yNow = new Date().getFullYear();
  for (let y = yNow + 1; y >= yNow - 15; y -= 1) {
    INV_RETRO_ANIOS.push({ value: y, label: String(y) });
  }
})();

const InventarioRetroactivoView = {
  _container: null,
  _rows: [],
  _totalCount: 0,
  ...
  mesLabel(mes) {
    return INV_RETRO_MESES.find((m) => m.value === Number(mes))?.label || String(mes);
  },
  ...
  renderTableBodyHtml(rows) {
    ...
    if (!rows.length) {
      const msg =
        this._filterQuery.trim() || this._filterMarca || this._filterHabilitado
          ? 'Ningún producto coincide con los filtros'
          : 'Sin saldo de inventario para el período seleccionado';
  ...
  badgeText() {
    ...
    const periodo = `al ${this.mesLabel(this._mes)} ${this._anio}`;
    return `<i class="fa-solid fa-clock-rotate-left me-1"></i>${countLabel} registro(s) · ${this.escapeHtml(periodo)}${this.filterSummaryHtml()}${this.escapeHtml(extra)}`;
  },
  ...
  apiUrlLista() {
    ...
    return `/api/inventario/retroactivo?${params.toString()}`;
  },

  exportUrl() {
    const params = this.buildListParams();
    return `/api/inventario/retroactivo/export?${params.toString()}`;
  },
      ...
      <div class="card inventario-filters-card shadow-sm mb-3">
        <div class="card-body">
          <div class="d-flex flex-wrap align-items-end gap-2 inventario-filters-row">
            <div class="inventario-filter-mes">
              <label for="inv-retro-mes" class="form-label small mb-1">Mes</label>
              <select class="form-select form-select-sm" id="inv-retro-mes">${this.renderMesOptions()}</select>
            </div>
            <div class="inventario-filter-anio">
              <label for="inv-retro-anio" class="form-label small mb-1">Año</label>
              <select class="form-select form-select-sm" id="inv-retro-anio">${this.renderAnioOptions()}</select>
            </div>
            <div class="inventario-filter-marca">
              <label for="inv-retro-filter-marca" class="form-label small mb-1">Marca</label>
              <select class="form-select form-select-sm" id="inv-retro-filter-marca">
                ${this.renderMarcaOptions()}
              </select>
            </div>
            <div class="inventario-filter-habilitado">
              <label for="inv-retro-filter-habilitado" class="form-label small mb-1">Habilitado</label>
              <select class="form-select form-select-sm" id="inv-retro-filter-habilitado">
            ...
            <div class="inventario-filter-search flex-grow-1">
              <label for="inv-retro-search" class="form-label small mb-1">Buscar</label>
              <div class="input-group input-group-sm">
                <span class="input-group-text" aria-hidden="true"><i class="fa-solid fa-magnifying-glass"></i></span>
                <input type="search" class="form-control" id="inv-retro-search"
                ...
                <button type="button" class="btn btn-outline-secondary" id="btn-inv-retro-search-clear"
            ...
            <div class="inventario-filter-export"${this.isUsuarioVentas() ? ' hidden' : ''}>
              ...
              <button type="button" class="btn btn-sm btn-outline-success" id="btn-inv-retro-export">
                <i class="fa-solid fa-file-excel me-1" aria-hidden="true"></i>Exportar (xlsx)
              </button>
            </div>
          </div>
          <div class="inventario-badge small text-muted mt-2" id="inv-retro-count">${this.badgeText()}</div>
          <div class="small text-muted mt-1">
            Saldo acumulado hasta el mes seleccionado = unidades × TIPOM de documentos no anulados.
            <em>Existencia</em> muestra el valor actual del producto.
          </div>
        </div>
      </div>
    `;
      ...
      <div class="card inventario-table-card shadow-sm">
        <div class="table-responsive">
          <table class="table table-sm table-hover table-striped mb-0">
            ...
            <tbody id="inv-retro-tbody">${this.renderTableBodyHtml(this._rows)}</tbody>
            <tfoot id="inv-retro-tfoot">${this.renderTableFooterHtml()}</tfoot>
          </table>
        </div>
      </div>
    `;
    ...
    return `
      <div class="inventario-vista-wrap">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-2">
          <div>
            <h2 class="h5 mb-0">Inventario Retroactivo</h2>
            <p class="small text-muted mb-0">Existencia calculada a una fecha contable (mes/año)</p>
          </div>
        </div>
        ${this.renderFiltersCard()}
        ${this.renderTableCard()}
        <button type="button" class="btn-onneb-nuevo-fab inventario-fab-print" id="btn-inv-retro-print"
          aria-label="Imprimir inventario retroactivo" title="Imprimir inventario retroactivo">
  ...
  syncFiltersFromUi() {
    const mesEl = document.getElementById('inv-retro-mes');
    const anioEl = document.getElementById('inv-retro-anio');
    const marcaEl = document.getElementById('inv-retro-filter-marca');
    const habilitadoEl = document.getElementById('inv-retro-filter-habilitado');
    if (mesEl) this._mes = Number(mesEl.value);
    if (anioEl) this._anio = Number(anioEl.value);
    if (marcaEl) this._filterMarca = marcaEl.value;
    if (habilitadoEl) this._filterHabilitado = habilitadoEl.value;
  },

  updateTableView() {
    const tbody = this._container?.querySelector('#inv-retro-tbody');
    const tfoot = this._container?.querySelector('#inv-retro-tfoot');
    const badge = this._container?.querySelector('#inv-retro-count');
  ...
  bindPeriodFilters() {
    ...
    mesEl?.addEventListener('change', () => {
      this._mes = Number(mesEl.value);
      this.reload();
    });
    anioEl?.addEventListener('change', () => {
      this._anio = Number(anioEl.value);
      this.reload();
    });
  },

  bindMarcaFilter() {
    const sel = document.getElementById('inv-retro-filter-marca');
    if (!sel) return;
  ...
  bindHabilitadoFilter() {
    const sel = document.getElementById('inv-retro-filter-habilitado');
    if (!sel) return;
    sel.addEventListener('change', () => {
  ...
  bindSearch() {
    const search = document.getElementById('inv-retro-search');
    const clearBtn = document.getElementById('btn-inv-retro-search-clear');
    if (!search) return;
  ...
  bindEvents() {
    ...
    this.bindSearch();
    this.bindTableRows();
    document.getElementById('btn-inv-retro-export')?.addEventListener('click', () => {
      this.onExportExcel();
    });
    document.getElementById('btn-inv-retro-print')?.addEventListener('click', () => {
      this.onPrint();
    });
  },
  ...
  async onExportExcel() {
    ...
    const btn = document.getElementById('btn-inv-retro-export');
    ...
    try {
      ...
      const stamp = `${this._anio}-${String(this._mes).padStart(2, '0')}`;
      const filename = match ? match[1] : `inventario_retroactivo_${empNit}_${stamp}.xlsx`;
    ...
    return PrintReport.wrapDocument({
      ...
      bodyHtml: `
        ${PrintReport.reportHeaderHtml({
          title: 'Reporte de inventario retroactivo',
          subtitleHtml: `
            <p><strong>Fecha impresión:</strong> ${PrintReport.escapeHtml(fecha)}</p>
            <p><strong>Período:</strong> ${PrintReport.escapeHtml(`${this.mesLabel(this._mes)} ${this._anio}`)}</p>
  ...
  async onPrint() {
    ...
    this.syncFiltersFromUi();
    const btn = document.getElementById('btn-inv-retro-print');
    this._printing = true;
    if (btn) btn.disabled = true;
    try {
      ...
      await PrintReport.openAndPrint(() => this.buildPrintHtml(rows, totals, Boolean(data.truncated)));
    } catch (err) {
  ...
  async reload() {
    ...
    if (tbody) {
      tbody.innerHTML = `<tr><td colspan="${this.tableColumns.length}" class="text-center text-muted py-4">
        <i class="fa-solid fa-spinner fa-spin me-2"></i>Calculando inventario…</td></tr>`;
    }
    try {
      ...
      F.toast('Error al cargar inventario retroactivo', 'error');
    } finally {
      this._loading = false;
    }
  },
  ...
  async load(container) {
    ...
    container.innerHTML = `
      <div class="text-center text-muted py-4 w-100">
        <i class="fa-solid fa-spinner fa-spin me-2"></i>Cargando inventario retroactivo…
      </div>
    `;
