/**
 * Vista Notas de crédito (Proveedores) — documentos DVP.
 * Basada en la UX de notas de crédito clientes, pero usando compra COM como referencia.
 */
const NotasDebitoView = {
  _container: null,
  _config: null,
  _pedido: null,
  _pedidosList: [],
  ...
  apiUrl(path, extraParams = {}) {
    ...
    return `/api/notas-debito${segment}?${params}`;
  },
  ...
  refFacturaHint() {
    return 'Solo compras tipo COM operadas';
  },
  ...
  readCodcajaForFinalizar() {
    const sel = this._container?.querySelector('#nd-doc-caja');
  ...
  renderCajaField() {
    ...
    return `
      <div class="pos-header-caja-wrap">
        <label class="form-label small mb-0" for="nd-doc-caja">Caja</label>
        <select class="form-select form-select-sm" id="nd-doc-caja"${disabled}>
  ...
  async fetchLineasDisponibles() {
    ...
    const data = await F.fetchJson(this.apiUrl(
      `/compras-referencia/${encodeURIComponent(serie)}/${encodeURIComponent(nofac)}/disponibles`,
      {
  ...
  async buscarComprasReferencia(q) {
    const coddocNota = this.activeCoddoc();
    return F.fetchJson(this.apiUrl('/compras-referencia', {
      q,
  ...
  async confirmarCompraReferencia(referencia, coddocDestino) {
    await this.crearPedidoDesdeFactura(referencia, coddocDestino);
    const key = this.docKey();
    F.toast('Nota de crédito proveedor creada', 'success');
  ...
  async onNuevoPedido() {
    ...
    if (!coddocDestino) {
      F.toast('Seleccione una serie DVP', 'warning');
      return;
    }
    const hint = this.refFacturaHint();
    const modal = await Swal.fire({
      ...CatalogosUI.modalBase(),
      title: 'Compra referencia',
      width: '52rem',
      html: `
        <div class="text-start">
          <p class="small text-muted mb-1">Busque la compra que servirá de referencia para la devolución al proveedor.</p>
          ${hint ? `<p class="small fw-semibold text-primary mb-2">${this.escapeHtml(hint)}</p>` : ''}
          <p class="small text-muted mb-2"><i class="fa-solid fa-hand-pointer me-1"></i>Doble clic en la compra para crear la nota.</p>
          <div class="input-group input-group-sm mb-2">
            <span class="input-group-text"><i class="fa-solid fa-magnifying-glass"></i></span>
            <input type="search" id="nd-ref-search" class="form-control" placeholder="Serie, número, proveedor, NIT…" autocomplete="off">
          </div>
          <div id="nd-ref-results" class="border rounded" style="max-height: 320px; overflow-y: auto;"></div>
        </div>
      `,
      ...
      focusConfirm: false,
      didOpen: () => {
        const qInp = document.getElementById('nd-ref-search');
        const box = document.getElementById('nd-ref-results');
        ...
        const renderRows = (rows) => {
          ...
          box.innerHTML = rows.map((r) => `
            <div class="list-group-item list-group-item-action border-0 border-bottom rounded-0 nd-ref-row"
              role="option"
              ...
              <div class="d-flex justify-content-between align-items-start gap-2">
                <div>
                  ...
                  <div class="small text-muted">${this.escapeHtml(r.DOC_NOMCLIE || 'Sin proveedor')}</div>
                </div>
        ...
        const fetchRows = F.debounce(async () => {
          ...
          try {
            const data = await this.buscarComprasReferencia(q);
            renderRows(data.rows || []);
          } catch (err) {
            box.innerHTML = `<p class="small text-danger text-center py-3 mb-0">${this.escapeHtml(err.message || 'Error al buscar compras')}</p>`;
          }
        }, 300);

        box?.addEventListener('dblclick', (e) => {
          const row = e.target.closest('.nd-ref-row');
          if (!row) return;
          const ref = {
    ...
    try {
      await this.confirmarCompraReferencia(modal.value, coddocDestino);
    } catch (err) {
      F.toast(err.message || 'No se pudo crear la nota de crédito proveedor', 'error');
    }
  },
    ...
    const picked = await Swal.fire({
      ...
      html: `
        <div class="text-start">
          ...
          <label class="form-label small mb-0" for="nd-swal-cant">Cantidad (máximo ${this.escapeHtml(String(disponible))})</label>
          <input type="number" id="nd-swal-cant" class="form-control form-control-sm" min="0.01" max="${this.escapeHtml(String(disponible))}" step="any" value="${disponible >= 1 ? 1 : disponible}">
        </div>
      `,
      showCancelButton: true,
      ...
      focusConfirm: false,
      preConfirm: () => {
        const cant = Number(document.getElementById('nd-swal-cant')?.value);
        if (!cant || cant <= 0) {
  ...
  async agregarTodasLineas() {
    ...
    const items = (this._lineasDisponibles || [])
      .map((row, idx) => {
        const input = this._container?.querySelector(`#nd-dev-qty-${idx}`);
  ...
  renderFinalizarFpagoCardHtml(totalPrecio) {
    ...
    return `
          <div class="card nd-finalizar-fpago-card h-100" id="nd-finalizar-fpago-card">
            ...
            <div class="card-body py-2 px-3 d-flex flex-column">
              ...
              <div class="row g-2 flex-grow-1">
                <div class="col-6">
                  <label class="form-label small mb-0" for="nd-finalizar-fpago-efectivo">Efectivo</label>
                  <input type="number" id="nd-finalizar-fpago-efectivo" class="form-control form-control-sm nd-fpago-input"
                    min="0" step="0.01" value="${total}">
                </div>
                <div class="col-6">
                  <label class="form-label small mb-0" for="nd-finalizar-fpago-tarjeta">Tarjeta</label>
                  <input type="number" id="nd-finalizar-fpago-tarjeta" class="form-control form-control-sm nd-fpago-input"
                    min="0" step="0.01" value="0">
                </div>
                <div class="col-6">
                  <label class="form-label small mb-0" for="nd-finalizar-fpago-deposito">Depósito</label>
                  <input type="number" id="nd-finalizar-fpago-deposito" class="form-control form-control-sm nd-fpago-input"
                    min="0" step="0.01" value="0">
                </div>
                <div class="col-6">
                  <label class="form-label small mb-0" for="nd-finalizar-fpago-cheque">Cheque</label>
                  <input type="number" id="nd-finalizar-fpago-cheque" class="form-control form-control-sm nd-fpago-input"
                    min="0" step="0.01" value="0">
                </div>
              </div>
              <div class="mt-2 small text-end text-muted" id="nd-finalizar-fpago-sum">Suma: ${this.escapeHtml(this.formatMoney(totalPrecio))} / ${this.escapeHtml(total)}</div>
              <div class="mt-2 mb-0">
                <label class="form-label small mb-0" for="nd-finalizar-fpago-desc">Detalles del pago</label>
                <input type="text" id="nd-finalizar-fpago-desc" class="form-control form-control-sm"
  ...
  sumFinalizarFpagoInputs() {
    const ids = [
      'nd-finalizar-fpago-efectivo',
      'nd-finalizar-fpago-tarjeta',
      'nd-finalizar-fpago-deposito',
      'nd-finalizar-fpago-cheque',
    ];
  ...
  bindFinalizarFpagoRefresh(totalPrecio) {
    const sumEl = document.getElementById('nd-finalizar-fpago-sum');
    const ids = [
      'nd-finalizar-fpago-efectivo',
      'nd-finalizar-fpago-tarjeta',
      'nd-finalizar-fpago-deposito',
  ...
  readFinalizarFpagoFromDom() {
    return {
      FPAGO_EFECTIVO: Number(document.getElementById('nd-finalizar-fpago-efectivo')?.value ?? 0),
      FPAGO_TARJETA: Number(document.getElementById('nd-finalizar-fpago-tarjeta')?.value ?? 0),
      FPAGO_DEPOSITO: Number(document.getElementById('nd-finalizar-fpago-deposito')?.value ?? 0),
      FPAGO_CHEQUE: Number(document.getElementById('nd-finalizar-fpago-cheque')?.value ?? 0),
      FPAGO_DESCRIPCION: document.getElementById('nd-finalizar-fpago-desc')?.value?.trim() || '',
    };
  },
  ...
  async finalizarPedido() {
    ...
    const ok = await Swal.fire({
      ...CatalogosUI.modalBase({
        customClass: { popup: 'modal-catalogo nd-finalizar-modal' },
      }),
      title: 'Finalizar nota de crédito proveedor',
      width: '52rem',
      html: `
        ...
        <div class="text-start nd-finalizar-modal-body">
          ...
          <div class="row g-3 align-items-stretch">
            <div class="col-md-6">
              ...
              <input type="text" id="nd-finalizar-obs" class="form-control form-control-sm"
                value="${obsVal}" placeholder="Motivo de la devolución" maxlength="500" autocomplete="off">
            </div>
            <div class="col-md-6" id="nd-finalizar-fpago-col">
    ...
    F.toast('Nota de crédito proveedor finalizada', 'success');
    this._pedido = null;
    await this.showList();
  },
      ...
      <div class="card shadow-sm">
        <div class="table-responsive">
          <table class="table table-sm table-hover table-striped mb-0">
            ...
            <tbody id="nd-list-tbody">${this.renderListTableBodyHtml()}</tbody>
          </table>
        </div>
      </div>`;
  },
    ...
    return `
      <div class="pos-list-wrap">
        <div class="pos-list-header">
          <h2 class="pos-list-title">Notas de crédito proveedor</h2>
        ...
        <div class="fac-list-toolbar mb-3">
          <div class="fac-list-toolbar-fecha">
            <label class="form-label small mb-1" for="nd-list-fecha">Fecha</label>
            <input type="date" class="form-control form-control-sm" id="nd-list-fecha"
              value="${this.escapeHtml(this._listFecha || this.todayIsoDate())}">
          </div>
          ${DocTipoSelect.renderSelectHtml({
            selectId: 'nd-list-coddoc',
          ...
          <div class="fac-list-toolbar-search flex-grow-1">
            <label class="form-label small mb-1" for="nd-list-search">Buscar</label>
            <div class="input-group input-group-sm">
              <span class="input-group-text"><i class="fa-solid fa-magnifying-glass"></i></span>
              <input type="search" class="form-control pos-search-glow" id="nd-list-search"
                placeholder="Documento, compra, proveedor…"
                value="${this.escapeHtml(this._listFilter)}" autocomplete="off">
            </div>
          </div>
        </div>
        ${this.renderListTableHtml()}
        <button type="button" class="btn-onneb-nuevo-fab pos-list-fab-nuevo" id="btn-nd-list-nuevo"
          aria-label="Nueva nota de crédito proveedor" title="Nueva nota de crédito proveedor"${this.activeCoddoc() ? '' : ' disabled'}>
  ...
  refLabel() {
    ...
    const serie = String(h.SERIEFAC || '').trim();
    const nofac = String(h.NOFAC || '').trim();
    const cliente = String(h.DOC_NOMCLIE || h.PROV_EMPRESA || h.PROV_RAZON || '').trim();
    const doc = serie && nofac ? `${serie}-${nofac}` : 'Sin referencia';
    return { doc, cliente: cliente || 'Sin proveedor' };
  },

  renderLineasDisponibles() {
    const target = this._container?.querySelector('#nd-product-list');
    if (!target) return;
    ...
    const rows = this._lineasDisponibles.map((ln, idx) => {
      ...
      const addBtn = editable
        ? `<button type="button" class="btn btn-sm btn-outline-primary" data-action="nd-add-line" data-idx="${idx}" title="Agregar"${canAdd ? '' : ' disabled'}>
      ...
      return `<tr>
        ...
        <td class="text-end" style="width: 6rem">
          ${editable
            ? `<input type="number" class="form-control form-control-sm text-end nd-dev-qty-input" id="nd-dev-qty-${idx}"
    ...
    target.innerHTML = `
      ${editable ? `
        <div class="d-flex justify-content-end gap-2 mb-2">
          <button type="button" class="btn btn-sm btn-outline-secondary" id="btn-nd-add-descuento"${this._cartBusy ? ' disabled' : ''}>
            <i class="fa-solid fa-tag me-1"></i>Agregar Descuento
          </button>
          <button type="button" class="btn btn-sm btn-primary" id="btn-nd-add-all"${this._cartBusy ? ' disabled' : ''}>
            <i class="fa-solid fa-list-check me-1"></i>Agregar todos
          </button>
        </div>` : ''}
      <div class="table-responsive">
        <table class="table table-sm table-hover align-middle mb-0 nd-disp-table">
  ...
  renderCart() {
    const tbody = this._container?.querySelector('#nd-cart-tbody');
    if (!tbody) return;
    const lines = this._pedido?.lines || [];
    const editable = this.docEditable(this._pedido?.header);
    if (!lines.length) {
      tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted py-3">Sin líneas en la nota de crédito proveedor</td></tr>';
      return;
    }
  ...
  renderOrderSummary() {
    const h = this._pedido?.header || {};
    const lines = this._pedido?.lines || [];
    const totalEl = this._container?.querySelector('#nd-header-total');
    const itemsEl = this._container?.querySelector('#nd-header-items');
    const docEl = this._container?.querySelector('#nd-header-doc');
      ...
      <div class="pos-vista-wrap">
        <div class="pos-header card shadow-sm">
          <div class="card-body pos-header-body">
            <div class="pos-header-top d-flex flex-wrap align-items-center gap-2">
              <button type="button" class="btn btn-sm btn-outline-secondary pos-btn-atras" id="btn-nd-atras">
            ...
            <div class="alert alert-info py-2 px-3 mt-2 mb-0">
              <div class="small mb-0"><strong>Compra referencia:</strong> ${this.escapeHtml(ref.doc)} · ${this.escapeHtml(ref.cliente)}</div>
            </div>
          </div>
        </div>
        <div class="pos-main">
          <div class="pos-panel pos-panel-search card shadow-sm">
            ...
            <div class="card-body">
              ...
              <div id="nd-product-list"></div>
            </div>
          </div>
          <div class="pos-panel pos-panel-cart card shadow-sm">
            <div class="card-header py-2 d-flex align-items-center gap-2">
              <i class="fa-solid fa-receipt"></i>
              <span class="fw-semibold">Nota de crédito proveedor actual</span>
            </div>
            <div class="card-body">
              <div class="small text-muted mb-2">Proveedor: <strong>${this.escapeHtml(ref.cliente)}</strong></div>
        ...
        ${editable ? `
          <div class="pos-fab-bar">
            <button type="button" class="btn btn-success pos-fab-btn" id="btn-nd-finalizar">
  ...
  refreshListDom() {
    const tbody = this._container?.querySelector('#nd-list-tbody');
  ...
  bindListEvents() {
    const search = this._container?.querySelector('#nd-list-search');
    search?.addEventListener('input', () => {
      this._listFilter = search.value;
      this.refreshListDom();
    });

    const fechaInp = this._container?.querySelector('#nd-list-fecha');
    ...
    this._container?.querySelector('#nd-list-tbody')?.addEventListener('click', async (e) => {
      ...
      e.preventDefault();
      e.stopPropagation();
      const row = btn.closest('.nd-list-row');
      if (!row) return;
    ...
    this._container?.querySelector('#btn-nd-list-nuevo')?.addEventListener('click', () => {
  ...
  setCartBusy(busy) {
    ...
    const addAll = this._container?.querySelector('#btn-nd-add-all');
    if (addAll) addAll.disabled = busy;
    const addDescuento = this._container?.querySelector('#btn-nd-add-descuento');
    if (addDescuento) addDescuento.disabled = busy;
    this._container?.querySelectorAll('[data-action="nd-add-line"]').forEach((btn) => {
      btn.disabled = busy;
    });
    this._container?.querySelectorAll('#nd-cart-tbody [data-action]').forEach((btn) => {
      btn.disabled = busy;
    });
  },

  bindEditorEvents() {
    this._container?.querySelector('#btn-nd-atras')?.addEventListener('click', () => this.showList());
    ...
    this._container?.querySelector('#nd-product-list')?.addEventListener('click', async (e) => {
      const descuentoBtn = e.target.closest('#btn-nd-add-descuento');
      if (descuentoBtn) {
        e.preventDefault();
      ...
      const addAllBtn = e.target.closest('#btn-nd-add-all');
      if (addAllBtn) {
        e.preventDefault();
      ...
      const btn = e.target.closest('[data-action="nd-add-line"]');
    ...
    this._container?.querySelector('#nd-product-list')?.addEventListener('input', (e) => {
      const inp = e.target.closest('.nd-dev-qty-input');
      if (!inp) return;
    ...
    this._container?.querySelector('#nd-cart-tbody')?.addEventListener('click', async (e) => {
  ...
  async load(container) {
    ...
    container.innerHTML = '<div class="text-center text-muted py-4 w-100"><i class="fa-solid fa-spinner fa-spin me-2"></i>Cargando notas de crédito proveedor…</div>';
    try {
      ...
      if (!this._config?.coddocDefault) {
        container.innerHTML = `
          <div class="alert alert-warning m-3 w-100">
            Configure un tipo de documento DVP activo para esta empresa.
          </div>`;
        return;
      }
