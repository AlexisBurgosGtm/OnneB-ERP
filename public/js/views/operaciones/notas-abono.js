...
const NotasAbonoView = {
  ...
  apiUrl(path, extraParams = {}) {
    ...
    return `/api/notas-abono${segment}?${params}`;
  },
  ...
  refFacturaHint() {
    ...
    if (t === 'FNA') return 'Solo facturas cambiarias FEC (FCAM)';
    return '';
  },
  ...
  readCodcajaForFinalizar() {
    const sel = this._container?.querySelector('#na-doc-caja');
  ...
  filterFacturasReferencia(rows) {
    return (rows || []).filter((r) => String(r?.TIPODOC || '').trim().toUpperCase() === 'FEC');
  },

  async confirmarFacturaReferencia(referencia, coddocDestino) {
    await this.crearPedidoDesdeFactura(referencia, coddocDestino);
    const key = this.docKey();
    F.toast('Nota de abono creada', 'success');
  ...
  async onNuevoPedido() {
    ...
    if (!coddocDestino) {
      F.toast('Seleccione una serie FNA', 'warning');
      return;
    }
    ...
    const modal = await Swal.fire({
      ...
      html: `
        <div class="text-start">
          <p class="small text-muted mb-1">Busque la factura cambiaria (FEC) de referencia para el abono.</p>
          ...
          <div class="input-group input-group-sm mb-2">
            ...
            <input type="search" id="na-ref-search" class="form-control" placeholder="Serie, número, cliente, NIT…" autocomplete="off">
          </div>
          <div id="na-ref-results" class="border rounded" style="max-height: 320px; overflow-y: auto;"></div>
        </div>
      `,
      ...
      focusConfirm: false,
      didOpen: () => {
        const qInp = document.getElementById('na-ref-search');
        const box = document.getElementById('na-ref-results');
        ...
        const renderRows = (rows) => {
          ...
          box.innerHTML = rows.map((r) => `
            <div class="list-group-item list-group-item-action border-0 border-bottom rounded-0 na-ref-row"
              role="option"
        ...
        const fetchRows = F.debounce(async () => {
          ...
          try {
            ...
            renderRows(this.filterFacturasReferencia(data.rows));
          } catch (err) {
        ...
        box?.addEventListener('dblclick', (e) => {
          const row = e.target.closest('.na-ref-row');
          if (!row) return;
          const ref = {
    ...
    try {
      ...
      F.toast(err.message || 'No se pudo crear la nota de abono', 'error');
    }
  },
  ...
  async agregarTodasLineas() {
    ...
    const items = (this._lineasDisponibles || [])
      .map((row, idx) => {
        const input = this._container?.querySelector(`#na-dev-qty-${idx}`);
  ...
  renderFinalizarFpagoCardHtml(totalPrecio) {
    ...
    return `
          <div class="card na-finalizar-fpago-card h-100" id="na-finalizar-fpago-card">
            ...
            <div class="card-body py-2 px-3 d-flex flex-column">
              ...
              <div class="row g-2 flex-grow-1">
                <div class="col-6">
                  <label class="form-label small mb-0" for="na-finalizar-fpago-efectivo">Efectivo</label>
                  <input type="number" id="na-finalizar-fpago-efectivo" class="form-control form-control-sm na-fpago-input"
                    min="0" step="0.01" value="${total}">
                </div>
                <div class="col-6">
                  <label class="form-label small mb-0" for="na-finalizar-fpago-tarjeta">Tarjeta</label>
                  <input type="number" id="na-finalizar-fpago-tarjeta" class="form-control form-control-sm na-fpago-input"
                    min="0" step="0.01" value="0">
                </div>
                <div class="col-6">
                  <label class="form-label small mb-0" for="na-finalizar-fpago-deposito">Depósito</label>
                  <input type="number" id="na-finalizar-fpago-deposito" class="form-control form-control-sm na-fpago-input"
                    min="0" step="0.01" value="0">
                </div>
                <div class="col-6">
                  <label class="form-label small mb-0" for="na-finalizar-fpago-cheque">Cheque</label>
                  <input type="number" id="na-finalizar-fpago-cheque" class="form-control form-control-sm na-fpago-input"
                    min="0" step="0.01" value="0">
                </div>
              </div>
              <div class="mt-2 small text-end text-muted" id="na-finalizar-fpago-sum">Suma: ${this.escapeHtml(this.formatMoney(totalPrecio))} / ${this.escapeHtml(total)}</div>
              <div class="mt-2 mb-0">
                <label class="form-label small mb-0" for="na-finalizar-fpago-desc">Detalles del pago</label>
                <input type="text" id="na-finalizar-fpago-desc" class="form-control form-control-sm"
  ...
  sumFinalizarFpagoInputs() {
    const ids = [
      'na-finalizar-fpago-efectivo',
      'na-finalizar-fpago-tarjeta',
      'na-finalizar-fpago-deposito',
      'na-finalizar-fpago-cheque',
    ];
  ...
  bindFinalizarFpagoRefresh(totalPrecio) {
    const sumEl = document.getElementById('na-finalizar-fpago-sum');
    const ids = [
      'na-finalizar-fpago-efectivo',
      'na-finalizar-fpago-tarjeta',
      'na-finalizar-fpago-deposito',
  ...
  readFinalizarFpagoFromDom() {
    return {
      FPAGO_EFECTIVO: Number(document.getElementById('na-finalizar-fpago-efectivo')?.value ?? 0),
      FPAGO_TARJETA: Number(document.getElementById('na-finalizar-fpago-tarjeta')?.value ?? 0),
      FPAGO_DEPOSITO: Number(document.getElementById('na-finalizar-fpago-deposito')?.value ?? 0),
      FPAGO_CHEQUE: Number(document.getElementById('na-finalizar-fpago-cheque')?.value ?? 0),
      FPAGO_DESCRIPCION: document.getElementById('na-finalizar-fpago-desc')?.value?.trim() || '',
    };
  },
  ...
  async finalizarPedido() {
    ...
    const ok = await Swal.fire({
      ...CatalogosUI.modalBase({
        customClass: { popup: 'modal-catalogo na-finalizar-modal' },
      }),
      title: 'Finalizar nota de abono',
      width: '52rem',
      html: `
        ...
        <div class="text-start na-finalizar-modal-body">
          <div class="row g-3 align-items-stretch">
            <div class="col-md-6">
              <label class="form-label small mb-0" for="na-finalizar-obs">Motivo de la devolución</label>
              <input type="text" id="na-finalizar-obs" class="form-control form-control-sm"
                value="${obsVal}" placeholder="Motivo de la devolución" maxlength="500" autocomplete="off">
            </div>
            <div class="col-md-6" id="na-finalizar-fpago-col">
      ...
      preConfirm: () => {
        ...
        return {
          OBS: document.getElementById('na-finalizar-obs')?.value?.trim() || '',
          CONCRE: 'CON',
  ...
  renderListTableBodyHtml() {
    ...
    if (!rows.length) {
      return `<tr><td colspan="10" class="text-center text-muted py-4">No hay notas de abono en esta fecha</td></tr>`;
    }
    return rows.map((r) => `
      <tr class="na-list-row" data-coddoc="${this.escapeHtml(r.CODDOC)}" data-correlativo="${r.CORRELATIVO}">
      ...
      <div class="card shadow-sm">
        <div class="table-responsive">
          <table class="table table-sm table-hover table-striped mb-0">
            ...
            <tbody id="na-list-tbody">${this.renderListTableBodyHtml()}</tbody>
          </table>
        </div>
      </div>`;
  },
    ...
    return `
      <div class="pos-list-wrap">
        <div class="pos-list-header">
          <h2 class="pos-list-title">Notas de Abono</h2>
        ...
        <div class="fac-list-toolbar mb-3">
          <div class="fac-list-toolbar-fecha">
            <label class="form-label small mb-1" for="na-list-fecha">Fecha</label>
            <input type="date" class="form-control form-control-sm" id="na-list-fecha"
              value="${this.escapeHtml(this._listFecha || this.todayIsoDate())}">
          </div>
          ${DocTipoSelect.renderSelectHtml({
            selectId: 'na-list-coddoc',
          ...
          <div class="fac-list-toolbar-search flex-grow-1">
            <label class="form-label small mb-1" for="na-list-search">Buscar</label>
            <div class="input-group input-group-sm">
              <span class="input-group-text"><i class="fa-solid fa-magnifying-glass"></i></span>
              <input type="search" class="form-control pos-search-glow" id="na-list-search"
        ...
        <button type="button" class="btn-onneb-nuevo-fab pos-list-fab-nuevo" id="btn-na-list-nuevo"
          aria-label="Nueva nota de abono" title="Nueva nota de abono"${this.activeCoddoc() ? '' : ' disabled'}>
  ...
  renderLineasDisponibles() {
    const target = this._container?.querySelector('#na-product-list');
    if (!target) return;
    ...
    const rows = this._lineasDisponibles.map((ln, idx) => {
      ...
      const addBtn = editable
        ? `<button type="button" class="btn btn-sm btn-outline-primary" data-action="na-add-line" data-idx="${idx}" title="Agregar"${canAdd ? '' : ' disabled'}>
      ...
      return `<tr>
        ...
        <td class="text-end" style="width: 6rem">
          ${editable
            ? `<input type="number" class="form-control form-control-sm text-end na-dev-qty-input" id="na-dev-qty-${idx}"
    ...
    target.innerHTML = `
      ${editable ? `
        <div class="d-flex justify-content-end gap-2 mb-2">
          <button type="button" class="btn btn-sm btn-outline-secondary" id="btn-na-add-descuento"${this._cartBusy ? ' disabled' : ''}>
            <i class="fa-solid fa-tag me-1"></i>Agregar Descuento
          </button>
          <button type="button" class="btn btn-sm btn-primary" id="btn-na-add-all"${this._cartBusy ? ' disabled' : ''}>
  ...
  renderCart() {
    const tbody = this._container?.querySelector('#na-cart-tbody');
    if (!tbody) return;
    const lines = this._pedido?.lines || [];
    const editable = this.docEditable(this._pedido?.header);
    if (!lines.length) {
      tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted py-3">Sin líneas en la nota de abono</td></tr>';
      return;
    }
  ...
  renderOrderSummary() {
    const h = this._pedido?.header || {};
    const lines = this._pedido?.lines || [];
    const totalEl = this._container?.querySelector('#na-header-total');
    const itemsEl = this._container?.querySelector('#na-header-items');
    const docEl = this._container?.querySelector('#na-header-doc');
      ...
      <div class="pos-vista-wrap">
        <div class="pos-header card shadow-sm">
          <div class="card-body pos-header-body">
            <div class="pos-header-top d-flex flex-wrap align-items-center gap-2">
              <button type="button" class="btn btn-sm btn-outline-secondary pos-btn-atras" id="btn-na-atras">
        ...
        <div class="pos-main">
          <div class="pos-panel pos-panel-search card shadow-sm">
            ...
            <div class="card-body">
              ...
              <div id="na-product-list"></div>
            </div>
          </div>
          <div class="pos-panel pos-panel-cart card shadow-sm">
            <div class="card-header py-2 d-flex align-items-center gap-2">
              <i class="fa-solid fa-receipt"></i>
              <span class="fw-semibold">Nota de abono actual</span>
            </div>
            <div class="card-body">
        ...
        ${editable ? `
          <div class="pos-fab-bar">
            <button type="button" class="btn btn-success pos-fab-btn" id="btn-na-finalizar">
  ...
  renderAll() {
    ...
    const editable = this.docEditable(this._pedido?.header);
    const fab = this._container?.querySelector('#btn-na-finalizar');
    if (fab) fab.style.display = editable ? '' : 'none';
  },

  refreshListDom() {
    const tbody = this._container?.querySelector('#na-list-tbody');
  ...
  async certificarPedido(coddoc, correlativo) {
    ...
    const confirm = await CatalogosUI.fireConfirm({
      title: 'Certificar nota de abono',
  ...
  bindListEvents() {
    const search = this._container?.querySelector('#na-list-search');
    search?.addEventListener('input', () => {
      this._listFilter = search.value;
      this.refreshListDom();
    });

    const fechaInp = this._container?.querySelector('#na-list-fecha');
    fechaInp?.addEventListener('change', async () => {
      ...
      try {
        ...
        F.toast(err.message || 'Error al cargar notas de abono', 'error');
      }
    });

    DocTipoSelect.bind(this._container, 'na-list-coddoc', this);

    this._container?.querySelector('#na-list-tbody')?.addEventListener('click', async (e) => {
      ...
      e.preventDefault();
      e.stopPropagation();
      const row = btn.closest('.na-list-row');
      if (!row) return;
    ...
    this._container?.querySelector('#btn-na-list-nuevo')?.addEventListener('click', () => {
  ...
  setCartBusy(busy) {
    ...
    const addAll = this._container?.querySelector('#btn-na-add-all');
    if (addAll) addAll.disabled = busy;
    const addDescuento = this._container?.querySelector('#btn-na-add-descuento');
    if (addDescuento) addDescuento.disabled = busy;
    this._container?.querySelectorAll('[data-action="na-add-line"]').forEach((btn) => {
      btn.disabled = busy;
    });
    this._container?.querySelectorAll('#na-cart-tbody [data-action]').forEach((btn) => {
      btn.disabled = busy;
    });
  },

  bindEditorEvents() {
    this._container?.querySelector('#btn-na-atras')?.addEventListener('click', () => this.showList());
    this._container?.querySelector('#na-doc-caja')?.addEventListener('change', (e) => {
    ...
    this._container?.querySelector('#na-product-list')?.addEventListener('click', async (e) => {
      const descuentoBtn = e.target.closest('#btn-na-add-descuento');
      if (descuentoBtn) {
        e.preventDefault();
      ...
      const addAllBtn = e.target.closest('#btn-na-add-all');
      if (addAllBtn) {
        e.preventDefault();
      ...
      const btn = e.target.closest('[data-action="na-add-line"]');
    ...
    this._container?.querySelector('#na-product-list')?.addEventListener('input', (e) => {
      const inp = e.target.closest('.na-dev-qty-input');
      if (!inp) return;
    ...
    this._container?.querySelector('#na-cart-tbody')?.addEventListener('click', async (e) => {
  ...
  async load(container) {
    ...
    container.innerHTML = '<div class="text-center text-muted py-4 w-100"><i class="fa-solid fa-spinner fa-spin me-2"></i>Cargando Notas de abono…</div>';
    try {
      ...
      if (!this._config?.coddocDefault) {
        container.innerHTML = `
          <div class="alert alert-warning m-3 w-100">
            Configure un tipo de documento de nota de abono activo (<strong>FNA</strong>) para esta empresa.
          </div>`;
        return;
      }
