...
const NotasCreditoView = {
  ...
  async finalizarPedido() {
    ...
    const ok = await Swal.fire({
      ...
      html: `
        <div class="text-start">
          <p class="small text-muted mb-2">${this.escapeHtml(this.docLabel())} · Total: <strong>${this.escapeHtml(this.formatMoney(h.TOTALPRECIO || 0))}</strong></p>
          <label class="form-label small mb-0" for="nc-finalizar-obs">Observaciones</label>
          <textarea id="nc-finalizar-obs" class="form-control form-control-sm" rows="3" placeholder="Observaciones...">${obsVal}</textarea>
        </div>
      `,
      icon: 'question',
      showCancelButton: true,
