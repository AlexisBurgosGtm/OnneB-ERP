...
router.get('/documentos/productos', async (req, res) => {
  ...
  if (!hasParty) {
    return res.status(400).json({ error: 'codcliente requerido' });
  }
  const partyFilter = partyFilterSql(req.query.codcliente, req.query.nit, req.query.nombre);
  ...
  try {
    ...
    console.warn('[API GET /cuentas-cobrar/documentos/productos]', err.message);
...
router.get('/documentos/lineas', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API GET /cuentas-cobrar/documentos/lineas]', err.message);
    res.status(500).json({ error: err.message });
  }
});
