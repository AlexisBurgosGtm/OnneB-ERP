...
} = require('../lib/notas-abono-disponible');
const {
...
const TIPODOC_NOTAS = [...TIPODOC_NOTAS_ABONO];
...
async function getTipoDocNotasAbono(pool, empnit, coddocPreferred) {
...
router.get('/config', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API GET /notas-abono/config]', err.message);
...
router.get('/vendedores', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API GET /notas-abono/vendedores]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/cajas-abiertas', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API GET /notas-abono/cajas-abiertas]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/pedidos', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API GET /notas-abono/pedidos]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/facturas-referencia', async (req, res) => {
  ...
  try {
    ...
    if (!tipodocNota) {
      return res.status(400).json({ error: 'Indique tipodoc_nota (FNA) o coddoc_nota de la serie' });
    }
    const tiposRef = TIPODOC_FACTURA_REF;
    ...
    console.warn('[API GET /notas-abono/facturas-referencia]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/facturas-referencia/:coddoc/:correlativo/disponibles', async (req, res) => {
  ...
  try {
    const pool = await req.app.locals.getDbPool();
    const factura = await loadFacturaReferencia(pool, empnit, coddoc, correlativo, TIPODOC_FACTURA_REF);
    if (!factura) {
    ...
    console.warn('[API GET /notas-abono/facturas-referencia/disponibles]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/pedidos/:coddoc/:correlativo', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API GET /notas-abono/pedidos/:coddoc/:correlativo]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/pedidos', async (req, res) => {
  ...
  const usuario = String(req.body?.USUARIO || req.body?.usuario || 'NA').trim();
  ...
  try {
    const pool = await req.app.locals.getDbPool();
    const tipo = await getTipoDocNotasAbono(pool, empnit, coddocBody);
    if (!tipo) {
      return res.status(400).json({
        error: `No hay tipo de documento de nota de abono (${TIPODOC_NOTAS.join(', ')}) activo para la empresa`,
      });
    }
    const tiposRef = TIPODOC_FACTURA_REF;
    ...
    try {
      ...
      await transaction
        ...
        .input('USUARIO', sql.VarChar, usuario || 'NA')
    ...
    console.warn('[API POST /notas-abono/pedidos]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.patch('/pedidos/:coddoc/:correlativo', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API PATCH /notas-abono/pedidos]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/pedidos/:coddoc/:correlativo/lineas', async (req, res) => {
  ...
  if (descuentoBody) {
    try {
      ...
      console.warn('[API POST /notas-abono/pedidos/lineas descuento]', err.message);
  ...
  try {
    ...
    try {
      ...
      const ent = valoresEntregadosIniciales(
        coddoc,
        lineTotals.totalUnidades,
        lineTotals.totalCosto,
        lineTotals.totalPrecio
      );
    ...
    console.warn('[API POST /notas-abono/pedidos/lineas]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.patch('/pedidos/:coddoc/:correlativo/lineas/:lineId', async (req, res) => {
  ...
  try {
    ...
    if (err.statusCode === 400) return res.status(400).json({ error: err.message });
    console.warn('[API PATCH /notas-abono/pedidos/lineas]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.delete('/pedidos/:coddoc/:correlativo/lineas/:lineId', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API DELETE /notas-abono/pedidos/lineas]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/pedidos/:coddoc/:correlativo/finalizar', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API POST /notas-abono/pedidos/finalizar]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.delete('/pedidos/:coddoc/:correlativo', async (req, res) => {
  ...
  try {
    ...
    console.warn('[API DELETE /notas-abono/pedidos]', err.message);
    res.status(500).json({ error: err.message });
  }
});
