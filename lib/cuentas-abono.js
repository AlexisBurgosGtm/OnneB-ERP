  ...
  const updRes = await transaction
    ...
    .query(`
      ...
      SET
        DOC_ABONO = @DOC_ABONO,
        DOC_SALDO = CASE
          WHEN ISNULL(TOTALPRECIO, 0) - @DOC_ABONO < 0 THEN 0
          ELSE ISNULL(TOTALPRECIO, 0) - @DOC_ABONO
        END
...
async function crearAbonoRcc(pool, sql, empnit, facCoddoc, facCorrelativo, body) {
  ...
  try {
    ...
    // Saldo efectivo: preferir DOC_SALDO; si está desfasado vs TOTAL-ABONO, usar el menor positivo.
    const docSaldoCol = roundMoney(toNumber(fac.DOC_SALDO));
    const docAbonoCol = roundMoney(toNumber(fac.DOC_ABONO));
    const saldoPorAbonos = roundMoney(Math.max(0, totalFac - docAbonoCol));
    const docSaldo = roundMoney(Math.min(docSaldoCol, saldoPorAbonos));
...
/**
 * Retenciones RVR/RIR aplicadas a FEL ligadas a la FAC (SERIEFAC/NOFAC),
 * que aún no se usaron como origen de un RAR sobre esa FAC.
 */
async function listRetencionesFelDeFac(pool, sql, empnit, facCoddoc, facCorrelativo) {
  ...
  const result = await pool
    ...
    .query(`
      ...
      INNER JOIN dbo.DOCUMENTOS fel
        ON fel.EMPNIT = a.EMPNIT AND fel.CODDOC = a.CODDOC_FAC AND fel.CORRELATIVO = a.CORRELATIVO_FAC
      ...
      WHERE a.EMPNIT = @EMPNIT
        ...
        AND tf.TIPODOC IN (${SQL_TIPODOC_FEL_CXC_IN})
        AND fel.STATUS = '${STATUS_OPERADO}'
        AND LTRIM(RTRIM(ISNULL(fel.SERIEFAC, ''))) = LTRIM(RTRIM(@SERIEFAC))
        AND (
          LTRIM(RTRIM(ISNULL(fel.NOFAC, ''))) = LTRIM(RTRIM(@NOFAC))
          OR TRY_CAST(LTRIM(RTRIM(fel.NOFAC)) AS DECIMAL(18, 0)) = @FAC_CORRELATIVO
        )
        ...
        AND NOT EXISTS (
          ...
          WHERE rar.EMPNIT = a.EMPNIT
            AND rar.STATUS = '${STATUS_OPERADO}'
            AND trar.TIPODOC = '${TIPODOC_RAR}'
            AND LTRIM(RTRIM(ISNULL(rar.SERIEFAC, ''))) = LTRIM(RTRIM(@SERIEFAC))
            AND (
              LTRIM(RTRIM(ISNULL(rar.NOFAC, ''))) = LTRIM(RTRIM(@NOFAC))
              OR TRY_CAST(LTRIM(RTRIM(rar.NOFAC)) AS DECIMAL(18, 0)) = @FAC_CORRELATIVO
            )
            AND CHARINDEX(
              '[' + LTRIM(RTRIM(ret.CODDOC)) + '#' + CAST(CAST(ret.CORRELATIVO AS BIGINT) AS VARCHAR(30))
              + '|' + LTRIM(RTRIM(fel.CODDOC)) + '#' + CAST(CAST(fel.CORRELATIVO AS BIGINT) AS VARCHAR(30)) + ']',
              ISNULL(rar.OBS, '')
            ) > 0
        )
  ...
  const retenciones = (result.recordset || []).map((r) => {
...
async function crearAbonoRar(pool, sql, empnit, facCoddoc, facCorrelativo, body) {
  ...
  if (!rawLineas.length) {
    const err = new Error('Seleccione al menos una retención para abonar');
    err.statusCode = 400;
    throw err;
  }
  ...
  for (const item of rawLineas) {
    ...
    if (!retCod || retCorr === null || !felCod || felCorr === null) {
      const err = new Error('Línea de retención inválida');
      err.statusCode = 400;
      throw err;
    }
    wanted.add(`${retCod}|${Math.trunc(retCorr)}|${felCod}|${Math.trunc(felCorr)}`);
  }
  ...
  const selected = disponibles.retenciones.filter((r) => wanted.has(r.KEY));
  if (selected.length !== wanted.size) {
    const err = new Error('Una o más retenciones ya no están disponibles para abonar a esta FAC');
    err.statusCode = 400;
    throw err;
  }
  ...
  try {
    ...
    const saldoPorAbonos = roundMoney(Math.max(0, totalFac - docAbonoCol));
    const docSaldo = roundMoney(Math.min(docSaldoCol, saldoPorAbonos));
    if (totalAbono > docSaldo + 0.001) {
      const err = new Error(
        `La suma de retenciones (${totalAbono}) supera el saldo de la FAC (${docSaldo})`
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    const tokens = selected.map((r) =>
      rarLineToken(r.CODDOC_RET, r.CORRELATIVO_RET, r.FEL_CODDOC, r.FEL_CORRELATIVO)
    );
    const obsRar = `${tokens.join(' ')} ${obsExtra || `Abono ret. ${facCod}-${correlativoFac}`}`.slice(
      0,
      240
    );
    ...
    await transaction
      ...
      .query(`
        INSERT INTO dbo.DOCUMENTOS (
          ...
          @FECHA, 0, 0, 0, 0, 0,
          @SERIEFAC, @NOFAC, NULL,
          0, 0, 0, 0, 'Abono por retenciones FEL'
        )
      `);
