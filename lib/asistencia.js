...
function parseQrPayload(raw, sessionEmpnit) {
  ...
  if (!text) throw new AsistenciaError('Código QR vacío');
  const parts = text.split('-');
  if (parts.length < 2) {
    throw new AsistenciaError('QR inválido. Use el formato empnit-codempleado');
  }
  const codempleado = parseInt(parts[parts.length - 1], 10);
  const empnit = parts.slice(0, -1).join('-').trim();
  if (!empnit || !Number.isFinite(codempleado) || codempleado <= 0) {
    throw new AsistenciaError('QR inválido. Use el formato empnit-codempleado');
  }
  if (String(sessionEmpnit).trim() !== empnit) {
    throw new AsistenciaError(
      `El QR pertenece a otra empresa (${empnit}). Sesión actual: ${sessionEmpnit}`
    );
  }
