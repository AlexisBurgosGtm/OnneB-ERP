 ...
 * Saldo por producto = SUM(TOTALUNIDADES × TIPOM). Sin bodegas.
 */
const LINE_MOV_CTE = `
  LineMov AS (
    SELECT
      ...
      CAST(ISNULL(l.TOTALUNIDADES, 0) AS FLOAT) * CAST(ISNULL(t.TIPOM, 0) AS FLOAT) AS DELTA,
      CASE
        WHEN CAST(ISNULL(l.TOTALUNIDADES, 0) AS FLOAT) * CAST(ISNULL(t.TIPOM, 0) AS FLOAT) > 0
        THEN CAST(ISNULL(l.TOTALUNIDADES, 0) AS FLOAT) * CAST(ISNULL(t.TIPOM, 0) AS FLOAT)
        ELSE 0
      END AS ENTRADAS,
      CASE
        WHEN CAST(ISNULL(l.TOTALUNIDADES, 0) AS FLOAT) * CAST(ISNULL(t.TIPOM, 0) AS FLOAT) < 0
        THEN ABS(CAST(ISNULL(l.TOTALUNIDADES, 0) AS FLOAT) * CAST(ISNULL(t.TIPOM, 0) AS FLOAT))
        ELSE 0
      END AS SALIDAS
