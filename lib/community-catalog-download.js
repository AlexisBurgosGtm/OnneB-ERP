 ...
 * Ítems: PRODUCTOS, PRECIOS + INVSALDO en cero.
 */
...
const { deleteOrphanInvSaldo, syncMissingInvSaldoZero } = require('./invsaldo');
...
async function downloadCatalogFromCommunity({ localPool, hostPool, token, empnit }) {
  ...
  const invsaldoCreados = await syncMissingInvSaldoZero(localPool, empnitVal);
