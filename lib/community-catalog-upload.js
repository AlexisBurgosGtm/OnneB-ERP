/**
 * Sube catálogo local (PRODUCTOS, PRECIOS, INVSALDO) al host
 * COMMUNITY_* usando bulk insert (equivalente a SqlBulkCopy).
 */
 ...
 * Orden: INVSALDO → PRECIOS → PRODUCTOS (hijos antes que padre).
 */
async function deleteCommunityCatalog(hostPool, token) {
  const tokenVal = String(token || '').trim();
  const tables = ['COMMUNITY_INVSALDO', 'COMMUNITY_PRECIOS', 'COMMUNITY_PRODUCTOS'];
  const deleted = {
    invsaldo: 0,
    precios: 0,
    productos: 0,
  ...
  for (const table of tables) {
    ...
    const result = await request
      ...
      .query(`
        DELETE FROM dbo.[${safeIdent(table)}]
    ...
    if (table === 'COMMUNITY_INVSALDO') deleted.invsaldo = n;
    else if (table === 'COMMUNITY_PRECIOS') deleted.precios = n;
    else if (table === 'COMMUNITY_PRODUCTOS') deleted.productos = n;
  }
...
async function uploadCatalogToCommunity({ localPool, hostPool, token, empnit }) {
  ...
  // 1) Borrar en nube (TOKEN + EMPNIT GENERAL) → 2) subir catálogo local
  ...
  for (const spec of TABLE_MAP) {
    ...
    const rows = localRes.recordset || [];

    const n = await bulkInsertRows(
      hostPool,
      spec.community,
      hostCols,
      rows,
    ...
    if (spec.local === 'PRODUCTOS') counts.productos = n;
    else if (spec.local === 'PRECIOS') counts.precios = n;
    else if (spec.local === 'INVSALDO') counts.invsaldo = n;
  }
