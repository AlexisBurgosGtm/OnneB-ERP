...
/** Saldo pendiente real: DOC_SALDO (restante) > 0. */
const SQL_SALDO_PENDIENTE_POSITIVO = 'ISNULL(d.DOC_SALDO, 0) > 0.005';
...
async function listMovimientosBanco(pool, sql, empnit, { q = '', limit = 200 } = {}) {
  ...
  const countRes = await pool
    .request()
    .input('EMPNIT', sql.VarChar, empnit)
    .query(`SELECT COUNT(1) AS total FROM dbo.DOCUMENTOS_BANCO WHERE EMPNIT = @EMPNIT`);
...
async function insertReciboYActualizar(
  ...
  await transaction
    ...
    .input('DOC_ABONO', sql.Decimal(18, 3), roundMoney(toNumber(fac.DOC_ABONO) + abono))
    .input('DOC_SALDO', sql.Decimal(18, 3), roundMoney(docSaldo - abono))
    .query(`
...
async function eliminarMovimientoBanco(pool, sql, empnit, id, { pass } = {}) {
  ...
  await assertAdminPass(pool, String(pass ?? ''));
