...
/**
 * Elimina documento operado sin corte de inventario (DOCPRODUCTOS + DOCUMENTOS).
 */
async function deleteDocumentoOperado(pool, empnit, coddoc, correlativo) {
  ...
  try {
    ...
    if (String(meta.CORTE || 'NO').trim().toUpperCase() === 'SI') {
      throw new DocumentoDeleteError('El documento ya fue finalizado; no se puede eliminar');
    }
