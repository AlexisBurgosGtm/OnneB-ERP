 ...
 * @param {Buffer} buffer
 * @returns {{ rows: Array<{ CODPROD: string, DESPROD: string, TOTALUNIDADES: number, excelRow: number }> }}
 */
function parseEntradaInventarioExcel(buffer) {
  ...
  dataRows.forEach((raw, idx) => {
    ...
    if (!codprod) {
      errors.push(`Fila ${excelRow}: falta CODPROD`);
      return;
    }
    if (totalUnidades === null) {
      errors.push(`Fila ${excelRow} (${codprod}): TOTALUNIDADES inválido`);
      return;
    }
    if (totalUnidades <= 0) {
      errors.push(`Fila ${excelRow} (${codprod}): TOTALUNIDADES debe ser mayor a cero`);
      return;
    }
  ...
  if (errors.length) {
    const err = new Error(errors.slice(0, 8).join('\n') + (errors.length > 8 ? `\n…y ${errors.length - 8} error(es) más` : ''));
    err.statusCode = 400;
    err.details = errors;
    throw err;
  }
  ...
  if (!rows.length) {
    const err = new Error('No hay filas de productos en el Excel (después del encabezado)');
    err.statusCode = 400;
    throw err;
  }
