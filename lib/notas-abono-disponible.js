...
/** Facturas de referencia permitidas para FNA (factura cambiaria / FCAM). */
const TIPODOC_FACTURA_REF = ['FEC'];
...
const SQL_TIPODOC_NOTAS_IN = TIPODOC_NOTAS_ABONO.map((t) => `'${t}'`).join(', ');

function sqlTipodocIn(tipos) {
  return (tipos || []).map((t) => `'${t}'`).join(', ');
}

/** FNA → solo facturas cambiarias (FEC). */
function tiposFacturaReferenciaParaNota(_tipodocNota) {
  return [...TIPODOC_FACTURA_REF];
}
...
function assertFacturaReferenciaPermitida(tipodocNota, tipodocFactura) {
  ...
  if (!permitidos.includes(fac)) {
    const err = new Error(
      'Las notas FNA solo pueden referenciar facturas cambiarias (FEC), no FAC ni otras series'
    );
    err.statusCode = 400;
    throw err;
  }
...
async function loadFacturaReferencia(pool, empnit, coddoc, correlativo, tipodocsRef = null) {
  const tipos = tipodocsRef && tipodocsRef.length ? tipodocsRef : TIPODOC_FACTURA_REF;
