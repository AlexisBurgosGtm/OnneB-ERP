...
const IMPUESTOS_CONTABILIDAD = [
  ...
  },
  {
    ...
    max: 100,
    description:
      'Porcentaje sobre la base gravada para recalcular documentos de retención IVA (RTV).',
  },
  {
    id: 'rti',
...
function splitIvaFromTotal(total, hasIva = true, ivaFactor = IVA_FACTOR_DEFAULT) {
  ...
  if (!hasIva || t <= 0 || factor <= 1) {
    return { gravable: 0, iva: 0, total: t };
  }
  const gravable = roundMoney(t / factor);
  const iva = roundMoney(t - gravable);
  return { gravable, iva, total: t };
}
