...
const ALL_MENUS = [
  ...
  'facturas-electronicas-extraordinarias',
...
const MENU_GROUPS = [
  ...
  { id: 'reportes', title: 'Reportes', menus: ['auditoria-cajas', 'reportes-ventas', 'reportes-clientes', 'reportes-productos', 'reportes-marcas', 'impresion-xml-fel', 'control-fletes'] },
  {
    id: 'online-services',
...
function ensureInicioInMenus(menuList) {
  const menus = [
    ...new Set(
      (menuList || [])
        .map((m) => String(m || '').trim())
        .filter((m) => ALL_MENUS_SET.has(m))
    ),
  ];
...
function normalizeAccesoMap(raw) {
  ...
  for (const [key, val] of Object.entries(src)) {
    ...
    out[cod] = ensureInicioInMenus(val);
  }
