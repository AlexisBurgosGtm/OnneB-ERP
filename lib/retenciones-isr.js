...
async function finalizarRetencionIsr(pool, empnit, coddoc, correlativo, body) {
  ...
  const totalIsr = roundMoney(body?.TOTALIVA ?? doc.TOTALIVA);
  const totalPrecio = roundMoney(body?.TOTALPRECIO ?? doc.TOTALPRECIO ?? totalIsr);
  if (totalPrecio <= 0) {
  ...
  const totalBase = roundMoney(body?.TOTALSINIVA ?? doc.TOTALSINIVA ?? 0);
  ...
  await pool
    ...
    .input('TOTALIVA', sql.Float, totalIsr)
