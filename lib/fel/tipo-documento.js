...
function isNabnNoCertificableTipodoc(tipodoc) {
  return TIPODOC_NABN_NO_CERTIFICABLE.has(normalizeTipodoc(tipodoc));
}
...
function assertCertificableTipodoc(tipodoc) {
  ...
  if (isNabnNoCertificableTipodoc(t)) {
    const err = new Error(
      'Las notas de abono (FNA) registran abonos internos contra facturas cambiarias (FEC). ' +
        'El DTE SAT NABN solo aplica a intermediarios agropecuarios de exportación autorizados (Art. 52-A IVA): ' +
        'requiere frases de exportación (tipo 9) y no admite el complemento cambiario ni el régimen EXE. ' +
        'Un contribuyente del régimen general (GEN) no puede certificar este documento como NABN. ' +
        'Consulte con Infile/SAT si su empresa tiene autorización específica.'
    );
    err.statusCode = 400;
    throw err;
  }
