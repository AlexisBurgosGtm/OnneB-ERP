...
async function loadDocumentoReferencia(pool, empnit, seriefac, nofac) {

  const corrRaw = Number(String(nofac).replace(/[^\d.-]/g, ''));

  if (!Number.isFinite(corrRaw)) {

    const err = new Error('NOFAC de referencia inválido');
  ...
  const result = await pool
    ...
    .query(`
      ...
      SELECT d.CODDOC, d.CORRELATIVO, d.FECHA, d.ANIO, d.MES, d.DIA,

        d.FEL_UUDI, d.FEL_SERIE, d.FEL_NUMERO, d.FEL_FECHA,
...
async function loadReferenciaNotaCredito(pool, empnit, header) {
  ...
  const orig = await loadDocumentoReferencia(pool, empnit, seriefac, nofac);
  ...
  if (!fechaIsoFromFelFecha(orig.FEL_FECHA)) {
    const err = new Error(
      'La factura original no tiene FEL_FECHA de SAT; no se puede certificar la nota de crédito'
    );
    err.statusCode = 400;
    throw err;
  }
