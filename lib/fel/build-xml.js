...
const { fechaIsoFromRow, fechaIsoFromFelFecha } = require('../documento-fecha');
const { satTipoFromTipodoc } = require('./tipo-documento');
const { buildAdendaXml } = require('./adendas');
const {
  escapeXml,
  cleanNit,
  formatFelDateTime,
...
function buildItemsXml(lines, ivaFactor = 1.12) {
  let totalIva = 0;
  const items = lines
    .map((line, index) => {
      ...
      const tax = splitIvaFromTotal(total, !exempt, ivaFactor);
      totalIva += tax.iva;
...
function buildComplementoAbonosCambiaria(header, numeroAbono = 1, idComplemento = '1') {
  const total = roundMoney(header.TOTALPRECIO || 0);
  const venc = formatFelDateOnly(header.VENCIMIENTO || header.FECHA);
  const numAbono = Math.max(1, Math.min(999, Math.trunc(Number(numeroAbono) || 1)));
  return `<dte:Complementos>
    <dte:Complemento IDComplemento="${escapeXml(String(idComplemento))}" NombreComplemento="Cambiaria" URIComplemento="${CFC_NS}">
      <cfc:AbonosFacturaCambiaria xmlns:cfc="${CFC_NS}" Version="1">
        <cfc:Abono>
          <cfc:NumeroAbono>${numAbono}</cfc:NumeroAbono>
...
function buildComplementoCambiaria(header) {
  return buildComplementoAbonosCambiaria(header, 1, '1');
}

function felDocumentoOrigenFecha(referencia) {
  // Prioridad: fecha leída del XML certificado en Infile/SAT; luego FEL_FECHA; no FECHA contable.
  const fromSat = String(referencia?.FEL_FECHA_EMISION_SAT || '').trim();
  if (/^\d{4}-\d{2}-\d{2}$/.test(fromSat)) return fromSat;

  const fecha = fechaIsoFromFelFecha(referencia?.FEL_FECHA);
  if (fecha) return fecha;
  const err = new Error(
    'La factura origen no tiene FEL_FECHA de SAT; no se puede armar FechaEmisionDocumentoOrigen'
  );
  err.statusCode = 400;
  throw err;
}
...
function buildComplementoReferenciasNota(header, referencia) {
  ...
  const numero = String(referencia.FEL_NUMERO || '').trim();
  if (!serie || !numero) {
    const err = new Error(
      'La factura origen no tiene FEL_SERIE/FEL_NUMERO de SAT; no se puede armar la nota de crédito'
    );
    err.statusCode = 400;
    throw err;
  }
  ...
  console.info(
    '[FEL NCRE] FechaEmisionDocumentoOrigen=',
    fecha,
    'FEL_FECHA=',
    String(referencia?.FEL_FECHA || '').trim()
  );
  return `<dte:Complementos>
    <dte:Complemento IDComplemento="ReferenciasNota" NombreComplemento="ReferenciasNota" URIComplemento="${CNO_NS}">
      <cno:ReferenciasNota xmlns:cno="${CNO_NS}" Version="1"
...
function felEmisionDateTime(header) {
  ...
  const iso = fechaIsoFromRow(header);
  const now = new Date();
  let hora = Number(header.HORA);
  let minuto = Number(header.MINUTO);
  if (!Number.isFinite(hora) || hora < 0 || hora > 23) hora = now.getHours();
  if (!Number.isFinite(minuto) || minuto < 0 || minuto > 59) minuto = now.getMinutes();
  const pad = (n) => String(n).padStart(2, '0');

  if (iso) {
    // Guatemala sin DST: -06:00. Evita desfase al construir desde DATE UTC.
    return `${iso}T${pad(hora)}:${pad(minuto)}:00-06:00`;
  }

  return formatFelDateTime(now);
}
...
function buildFelXml({ empnit, cred, header, lines, referencia, ivaFactor = 1.12, adendaContext }) {
  ...
  const { itemsXml, totalIva } = buildItemsXml(lines, ivaFactor);
  ...
  const afiliacion = afiliacionIva(cred, totalIva);
  const adendaXml = buildAdendaXml(header, adendaContext);
  ...
  <dte:SAT ClaseDocumento="dte">
    <dte:DTE ID="DatosCertificados">
      <dte:DatosEmision ID="DatosEmision">
        ...
        <dte:Emisor AfiliacionIVA="${afiliacion}" CodigoEstablecimiento="${escapeXml(cred.EMISOR_CODIGOESTABLECIMIENTO || '1')}" CorreoEmisor="" NITEmisor="${escapeXml(cleanNit(cred.EMISOR_NIT))}" NombreComercial="${escapeXml(cred.EMISOR_NOMBRECOMECIAL || cred.EMISOR_NOMBRE)}" NombreEmisor="${escapeXml(cred.EMISOR_NOMBRE)}">
          <dte:DireccionEmisor>
