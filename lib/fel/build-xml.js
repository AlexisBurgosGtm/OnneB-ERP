const { FEL_NS, CFC_NS, FEL_ANULACION_NS } = require('./constants');
...
function buildReferenciasNota(header, referencia) {
  ...
  const fecha = formatFelDateOnly(referencia.FECHA);
  const motivo = String(header.OBS || 'Anulación / ajuste').trim() || 'Ajuste';
  return `<dte:ReferenciasNota>
    <dte:ReferenciaNota
...
function buildFelXml({ empnit, cred, header, lines, referencia }) {
  ...
  if (satTipo === 'NCRE' && referencia) extraBlocks += buildReferenciasNota(header, referencia);
