...
const { fechaIsoFromRow, fechaIsoFromValue, calendarPartsInTimeZone, dateOnlyString } = require('../documento-fecha');
...
function felDocumentoOrigenFecha(referencia) {
  const raw = referencia?.FEL_FECHA;
  if (raw instanceof Date && !Number.isNaN(raw.getTime())) {
    if (raw.getUTCHours() === 0 && raw.getUTCMinutes() === 0 && raw.getUTCSeconds() === 0) {
      return dateOnlyString(raw.getUTCFullYear(), raw.getUTCMonth() + 1, raw.getUTCDate());
    }
    return calendarPartsInTimeZone(raw).fecha;
  }
  const felFecha = String(raw || '').trim();
  if (felFecha) {
    if (/T/.test(felFecha) || /Z$/i.test(felFecha) || /[+-]\d{2}:\d{2}/.test(felFecha)) {
      const d = new Date(felFecha);
      if (!Number.isNaN(d.getTime())) return calendarPartsInTimeZone(d).fecha;
    }
    const iso = felFecha.match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (iso) return `${iso[1]}-${iso[2]}-${iso[3]}`;
    const dmy = felFecha.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})/);
    if (dmy) return dateOnlyString(Number(dmy[3]), Number(dmy[2]), Number(dmy[1]));
  }
  return fechaIsoFromRow(referencia) || fechaIsoFromValue(referencia?.FECHA) || '';
}
