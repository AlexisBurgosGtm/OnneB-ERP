...
/** RTV/RTI (retenciones) no se integran al libro de compras. */
...
async function listLibroCompras(pool, sql, empnit, mes, anio) {
  ...
  const result = await pool
    ...
    .query(`
      ...
      ORDER BY
        CASE
          WHEN d.FEL_FECHA IS NOT NULL AND LTRIM(RTRIM(d.FEL_FECHA)) <> '' THEN TRY_CONVERT(DATE, d.FEL_FECHA)
          ELSE CAST(d.FECHA AS DATE)
        END ASC,
        d.ID ASC
    `);
  ...
  let manualRows = [];
  try {
    manualRows = (await fetchManualAsLibroRaw(pool, sql, empnit, mes, anio, LIBRO_COMPRAS)).filter(
      (r) => TIPODOC_LIBRO_COMPRAS.includes(String(r.TIPODOC || '').trim().toUpperCase())
    );
  } catch (err) {
    console.warn('[libro-compras] registros manuales:', err.message);
  }
  const merged = sortLibroRawRows([...docRows, ...manualRows]);
  const rows = merged.map((row, index) => mapLibroComprasRow(row, index, ivaFactor));
  const totals = summarizeRows(rows);
  totals.manuales = manualRows.length;
  return {
    rows,
    totals,
    mes,
