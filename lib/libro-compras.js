...
function isRetencion(row) {
  const t = String(row?.TIPODOC ?? '').trim().toUpperCase();
  return t === 'RTV' || t === 'RTI';
}
 ...
 * Retenciones (RTV/RTI) conservan montos del documento (no son IVA 12%).
 */
function desgloseIvaLibroCompras(row, ivaFactor) {
  ...
  return desgloseIvaIncluyente(resolveTotal(row), toNumber(row.TOTALEXENTO), ivaFactor);
}
  ...
  const result = await pool
    ...
    .query(`
      ...
      WHERE d.EMPNIT = @EMPNIT
        ...
        AND t.TIPODOC IN ('COM', 'COP', 'DVP', 'RTV', 'RTI')
      ORDER BY
        CASE
