...
async function crearPagoRcp(pool, sql, empnit, comCoddoc, comCorrelativo, body) {
  ...
  try {
    ...
    const docSaldoCol = roundMoney(toNumber(com.DOC_SALDO));
    const docAbonoCol = roundMoney(toNumber(com.DOC_ABONO));
    const saldoPorAbonos = roundMoney(Math.max(0, totalCom - docAbonoCol));
