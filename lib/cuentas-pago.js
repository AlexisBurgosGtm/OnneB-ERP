...
async function crearPagoRcp(pool, sql, empnit, comCoddoc, comCorrelativo, body) {
  ...
  try {
    ...
    const docSaldo = toNumber(com.DOC_SALDO);
    const docAbono = toNumber(com.DOC_ABONO);
    ...
    return {
      ...
      compra: {
        ...
        SALDO_PENDIENTE: roundMoney(nuevoSaldo - nuevoAbono),
      },
    };
  } catch (err) {
 ...
 * más abonos de DOCUMENTOS_FACTURAS_ABONADAS cuyo recibo asociado no esté operado.
 */
async function corregirSaldosCxp(pool, sql, empnit) {
  ...
  return {
    ok: true,
    totalCompras,
    actualizadas: Number(updRes.rowsAffected[0]) || 0,
  };
}
