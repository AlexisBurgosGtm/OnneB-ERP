...
const {
  RVR_CODDOC,
  RVR_TIPODOC,
  ensureRetencionesIvaRecibidasSetup,
} = require('./retenciones-iva-recibidas-setup');
const {
  loadCalcParams,
...
async function getTipoDocRvr(pool, empnit) {
  const result = await pool
    .request()
    .input('EMPNIT', sql.VarChar, empnit)
    .input('CODDOC', sql.VarChar, RVR_CODDOC)
    .query(`
      SELECT CODDOC, DESDOC, TIPODOC, CORRELATIVO, CODFORMATOCON, CODFORMATOCRE, CONTABLE
      FROM dbo.TIPODOCUMENTOS
      WHERE EMPNIT = @EMPNIT AND CODDOC = @CODDOC AND TIPODOC = '${RVR_TIPODOC}' AND ACTIVO = 'SI'
    `);
...
function clienteDisplayName(cli) {
  if (!cli) return '';
  return String(cli.NOMBRECLIENTE || cli.NEGOCIO || '').trim();
}
...
async function loadDocumento(pool, empnit, coddoc, correlativo) {
  ...
  doc.calc = await loadCalcParams(pool, 'iva');
  return doc;
}

async function listRetencionesIvaRecibidas(pool, empnit, mes, anio) {
  const result = await pool
    .request()
    .input('EMPNIT', sql.VarChar, empnit)
    ...
    .query(`
      ...
      WHERE d.EMPNIT = @EMPNIT
        AND d.MES = @MES
        AND d.ANIO = @ANIO
        AND t.TIPODOC = '${RVR_TIPODOC}'
      ORDER BY d.FECHA DESC, d.CORRELATIVO DESC
    `);
  return result.recordset.map(mapDocumentoRow);
}

async function createRetencionIvaRecibida(pool, empnit, body) {
  await ensureRetencionesIvaRecibidasSetup(pool, empnit);
  const tipo = await getTipoDocRvr(pool, empnit);
  if (!tipo) {
    const err = new Error('Tipo de documento RVR no configurado');
    err.statusCode = 400;
    throw err;
  }
  const codcliente = parseInt(body?.CODCLIENTE, 10);
  let cliente = null;
  if (!Number.isNaN(codcliente)) cliente = await getClienteSnapshot(pool, empnit, codcliente);
  if (!cliente) cliente = await getClienteSnapshot(pool, empnit, 1);
  if (!cliente) {
    const err = new Error('No hay cliente disponible');
    err.statusCode = 400;
    throw err;
  }
  ...
  try {
    const correlativo = await allocateCorrelativo(transaction, empnit, RVR_CODDOC);
    const nom = clienteDisplayName(cliente) || 'CLIENTE';
    ...
    await transaction
      ...
      .query(`
        INSERT INTO dbo.DOCUMENTOS (
          ...
          @EMPNIT, @ANIO, @MES, @DIA, @FECHA, @HORA, @MINUTO, @CODDOC, @CORRELATIVO,
          @CODCLIENTE, @DOC_NIT, @DOC_NOMCLIE, @DOC_DIRCLIE,
          0, 0, '${CODEMBARQUE_RVR}', '${STATUS_OPERADO}', @USUARIO, 'CON', 'NO',
          'SN', @OBS, 0, 0, 'SN', 0, 1,
    ...
    return loadDocumento(pool, empnit, RVR_CODDOC, correlativo);
  } catch (err) {
...
async function updateRetencionIvaRecibida(pool, empnit, coddoc, correlativo, body) {
  const updates = [];
  const request = pool
    .request()
    .input('EMPNIT', sql.VarChar, empnit)
  ...
  if (body?.CODCLIENTE !== undefined) {
    ...
    if (Number.isNaN(codcliente)) {
      const err = new Error('CODCLIENTE inválido');
      err.statusCode = 400;
      throw err;
    }
    ...
    request.input('DOC_NOMCLIE', sql.VarChar, clienteDisplayName(cliente));
...
async function finalizarRetencionIvaRecibida(pool, empnit, coddoc, correlativo, body) {
  ...
  const calc = doc.calc || (await loadCalcParams(pool, 'iva'));
  ...
  if (
    ...
    body?.OBS !== undefined ||
    body?.CONCRE !== undefined
  ) {
    await updateRetencionIvaRecibida(pool, empnit, coddoc, correlativo, {
  ...
  await finalizarRetencionConAbonos(pool, {
    ...
    coddoc,
    correlativo,
    codcliente: body?.CODCLIENTE ?? doc.CODCLIENTE,
    fechaStr,
...
async function searchClientes(pool, empnit, q, limit = 30) {
  ...
  const result = await pool
    ...
    .query(`
      SELECT TOP (@limit) CODCLIENTE, NIT, NOMBRECLIENTE, DIRCLIENTE, NEGOCIO
      FROM dbo.CLIENTES
      WHERE EMPNIT = @EMPNIT
        AND (
          @q IS NULL OR @q = ''
          OR CAST(CODCLIENTE AS VARCHAR(20)) LIKE @qLike
          OR NIT LIKE @qLike
          OR NOMBRECLIENTE LIKE @qLike
          OR ISNULL(NEGOCIO, '') LIKE @qLike
        )
      ORDER BY NOMBRECLIENTE, CODCLIENTE
    `);
...
module.exports = {
  ...
  ensureRetencionesIvaRecibidasSetup,
  getTipoDocRvr,
  listRetencionesIvaRecibidas,
  createRetencionIvaRecibida,
  updateRetencionIvaRecibida,
  finalizarRetencionIvaRecibida,
  loadDocumento,
  searchClientes,
  parseCorrelativo,
  listFacturasCreditoPendientesCliente,
