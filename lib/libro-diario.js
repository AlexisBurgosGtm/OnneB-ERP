...
const { fetchAsientosDiarioLines } = require('./conta-asientos');
...
async function listLibroDiario(pool, sql, empnit, mes, anio) {
  ...
  let manualMeta = { asientosCount: 0, anuladosCount: 0 };
  try {
    const manual = await fetchAsientosDiarioLines(pool, sql, empnit, mes, anio, lineNo);
    lines.push(...manual.lines);
    lineNo = manual.nextLine;
    manualMeta = manual;
  } catch (err) {
    // Si las tablas aún no existen en BD local, el Diario de documentos sigue funcionando.
    console.warn('[libro-diario] asientos manuales:', err.message);
  }

  const totals = summarizeLines(lines);
  totals.documentos = docs.length + (manualMeta.asientosCount || 0);
  totals.anulados =
    docs.filter((d) => isAnulado(d)).length + (manualMeta.anuladosCount || 0);
  ...
  totals.asientosManuales = manualMeta.asientosCount || 0;
