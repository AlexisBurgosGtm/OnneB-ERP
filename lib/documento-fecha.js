...
/**
 * Fecha calendario (YYYY-MM-DD) desde DOCUMENTOS.FEL_FECHA.
 * Preferir el prefijo ISO literal (sin reinterpretar TZ): SAT compara contra
 * la fecha embebida en FechaHoraEmision/FEL del DTE, no contra el día local
 * recalculado (p. ej. 2026-08-06T02:00:00Z ≠ 2026-08-05 en Guatemala).
 */
function fechaIsoFromFelFecha(raw) {
  ...
  if (raw instanceof Date) {
    ...
    // Date sin representación ISO original: no desplazar a otra zona.
    return dateOnlyString(raw.getUTCFullYear(), raw.getUTCMonth() + 1, raw.getUTCDate());
  }
