...
async function createVale(pool, empnit, data) {
  ...
  try {
    ...
    const codcajaInsert = generadoTipo === 'CAJA' ? generadoCodigo : null;

    const insert = await pool
      .request()
      .input('EMPNIT', sql.VarChar, empnit)
      .input('CODEMP', sql.Int, codemp)
      .input('CODCAJA', sql.Int, codcajaInsert)
    ...
    if (/CODCAJA/i.test(msg) && /NULL/i.test(msg)) {
      throw httpError(
        'La columna CODCAJA de NOMINA_VALES_EMPLEADOS no admite NULL. Ejecute el script SQL que la deja nullable (scripts/sql/dbo.NOMINA_VALES_EMPLEADOS_CODCAJA_NULL.sql).',
        500
      );
    }
    throw err;
