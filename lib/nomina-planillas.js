...
async function listEmpleadosActivos(pool, empnit) {
  const result = await pool.request().input('EMPNIT', sql.VarChar, empnit).query(`
    SELECT
      e.CODEMPLEADO, e.NOMEMPLEADO, e.DPI, e.IGSS, e.ACTIVO,
      ...
      ne.TIEMPO_COMPLETO, ne.HORAS_MES, ne.COD_OCUPACION_IGSS, ne.CUENTA_BANCO, ne.OBS
    ...
    await pool
      ...
      .query(`
        UPDATE dbo.NOMINA_EMPLEADO SET
          ...
          CUENTA_BANCO=@CUENTA_BANCO, OBS=@OBS
...
async function createPlanilla(pool, empnit, tipo, body) {
  ...
  const diasMes = toNumber(config.DIAS_MES, 30);
  ...
  try {
    const ins = await transaction
      ...
      .input('EMPNIT', sql.VarChar, empnit)
      .input('TIPO', sql.VarChar, tipo)
      .input('PERIODO_TIPO', sql.VarChar, String(body.PERIODO_TIPO || 'MENSUAL'))
      .input('MES', sql.Int, mes)
      .input('ANIO', sql.Int, anio)
    ...
    for (const emp of empleados) {
      const raw = buildLineFromEmpleado(emp, config, diasMes);
      ...
      await transaction
        ...
        .query(`
          INSERT INTO dbo.NOMINA_DETALLE (
            ...
            HORAS_LABORADAS, OTROS_INGRESOS, BONIFICACION, COMISION, IGSS_LABORAL, IGSS_PATRONAL, ISR,
            OTRAS_DEDUCCIONES, TOTAL_INGRESOS, TOTAL_DEDUCCIONES, NETO_PAGAR, FECHA_ALTA, FECHA_BAJA,
            COD_CENTRO_TRABAJO, CONDICION_LABORAL, TIPO_SALARIO_IGSS, TIEMPO_COMPLETO,
            COD_OCUPACION_IGSS, INCLUIDO, OBS
          ) VALUES (
            @PLANILLA_ID, @EMPNIT, @CODEMPLEADO, @NOMEMPLEADO, @DPI, @IGSS, @SALARIO_BASE, @DIAS_LABORADOS,
            @HORAS_LABORADAS, @OTROS_INGRESOS, @BONIFICACION, @COMISION, @IGSS_LABORAL, @IGSS_PATRONAL, @ISR,
            @OTRAS_DEDUCCIONES, @TOTAL_INGRESOS, @TOTAL_DEDUCCIONES, @NETO_PAGAR, @FECHA_ALTA, @FECHA_BAJA,
  ...
  await pool
    ...
    .query(`
      UPDATE dbo.NOMINA_DETALLE SET
        ...
        OTROS_INGRESOS=@OTROS_INGRESOS, BONIFICACION=@BONIFICACION, COMISION=@COMISION,
        IGSS_LABORAL=@IGSS_LABORAL, IGSS_PATRONAL=@IGSS_PATRONAL, ISR=@ISR,
        OTRAS_DEDUCCIONES=@OTRAS_DEDUCCIONES, TOTAL_INGRESOS=@TOTAL_INGRESOS,
        TOTAL_DEDUCCIONES=@TOTAL_DEDUCCIONES, NETO_PAGAR=@NETO_PAGAR, INCLUIDO=@INCLUIDO, OBS=@OBS
      WHERE ID=@ID
    `);
