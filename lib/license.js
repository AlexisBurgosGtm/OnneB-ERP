...
const LICENSE_PATH = path.join(ROOT, 'data', 'license.json');
const PUBLIC_KEY_PATH = path.join(ROOT, 'config', 'license-public.pem');
...
function evaluateLicense() {
  ...
  if (!raw) {
    ...
    return {
      mode: 'open',
      status: 'open',
      message: 'Sin archivo de licencia — instalación en modo abierto',
...
function isMenuLicensed(menuKey) {
  ...
  if (!key) return false;
  if (CORE_MENUS.includes(key) || key === 'licencia') return true;
