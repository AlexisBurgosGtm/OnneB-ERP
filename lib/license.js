...
function evaluateLicense() {
  ...
  const payloadMenus = Array.isArray(verified.payload.menus)
    ? verified.payload.menus.map((m) => String(m || '').trim()).filter(Boolean)
    : [];
  const menusStripped = payloadMenus.filter((m) => !menus.includes(m));
  if (menusStripped.length) {
    console.warn(
      `[Licencia] ${menusStripped.length} vista(s) del archivo no reconocidas por esta versión del ERP: ${menusStripped.join(', ')}`
    );
  }
  const modules =
  ...
  return {
    ...
    modules,
    menus,
    menusStripped: menusStripped.length ? menusStripped : undefined,
    customer: verified.payload.customer,
    expiresAt: verified.payload.expiresAt,
    licenseId: verified.payload.licenseId,
    issuedAt: verified.payload.issuedAt,
...
function publicStatusPayload() {
  ...
  return {
    ...
    modules: st.modules,
    menus: st.menus,
    menusStripped: st.menusStripped,
