...
function mapLibroVentasRow(row, index) {
  ...
  const total = roundMoney(toNumber(row.TOTALPRECIO) * sign);
...
async function listLibroVentas(pool, sql, empnit, mes, anio) {
  ...
  const rows = result.recordset.map((row, index) => mapLibroVentasRow(row, index));
  return {
    rows,
