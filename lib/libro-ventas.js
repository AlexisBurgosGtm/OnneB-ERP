...
async function listLibroVentas(pool, sql, empnit, mes, anio) {
  ...
  let manualRows = [];
  try {
    manualRows = (await fetchManualAsLibroRaw(pool, sql, empnit, mes, anio, LIBRO_VENTAS)).filter(
      (r) => TIPODOC_LIBRO_VENTAS.includes(String(r.TIPODOC || '').trim().toUpperCase())
    );
  } catch (err) {
    console.warn('[libro-ventas] registros manuales:', err.message);
  }
  const merged = sortLibroRawRows([...docRows, ...manualRows]);
  const rows = merged.map((row, index) => mapLibroVentasRow(row, index, ivaFactor));
