...
function buildProyeccion30(ventasPorDia, anio, mes) {
  const daysWithActivity = ventasPorDia.filter(
    (d) => d.ventas !== 0 || d.devoluciones !== 0 || d.neto !== 0
  );
  const baseDays = daysWithActivity.length || ventasPorDia.length || 1;
  const totalNeto = ventasPorDia.reduce((s, d) => s + d.neto, 0);
  const promedioDiario = totalNeto / baseDays;

  const lastDay = daysInMonth(anio, mes);
  const start = new Date(anio, mes - 1, lastDay);
  const futuro = [];
  ...
  return {
    ...
    totalProyectado30: Math.round(promedioDiario * 30 * 100) / 100,
    diasBase: baseDays,
    historico: ventasPorDia.map((d) => ({ ...d, tipo: 'real' })),
    futuro,
  };
}
...
async function loadAdminDashboard(pool, sql, empnit, mes, anio) {
  ...
  const proyeccion = buildProyeccion30(ventasPorDia, anio, mes);
