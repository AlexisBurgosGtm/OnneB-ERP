...
const RVR_CODDOC = 'RVR';
const RVR_TIPODOC = 'RVR';
const RVR_FORMATO_CON = 'RVRCON';
const RVR_FORMATO_CRE = 'RVRCRE';
...
async function ensureConfigTipodoc(pool) {
  try {
    const exists = await pool.request().input('TIPODOC', sql.VarChar, RVR_TIPODOC).query(`
    ...
    await pool
      .request()
      .input('TIPODOC', sql.VarChar, RVR_TIPODOC)
      .input('DESCRIPCION', sql.VarChar, 'RETENCIONES IVA RECIBIDAS')
      .query(`
...
async function ensureTipoDocumentoRvr(pool, empnit) {
  ...
  if (exists.recordset.length) {
    await pool
      ...
      .input('EMPNIT', sql.VarChar, empnit)
      .input('CODDOC', sql.VarChar, RVR_CODDOC)
      .input('CODFORMATOCON', sql.VarChar, RVR_FORMATO_CON)
      .input('CODFORMATOCRE', sql.VarChar, RVR_FORMATO_CRE)
      .query(`
  ...
  await pool
    ...
    .input('EMPNIT', sql.VarChar, empnit)
    .input('CODDOC', sql.VarChar, RVR_CODDOC)
    .input('DESDOC', sql.VarChar, 'RETENCIONES IVA RECIBIDAS')
    .input('CODFORMATOCON', sql.VarChar, RVR_FORMATO_CON)
    .input('CODFORMATOCRE', sql.VarChar, RVR_FORMATO_CRE)
    .query(`
      INSERT INTO dbo.TIPODOCUMENTOS (
        ...
        @EMPNIT, @CODDOC, @DESDOC, '${RVR_TIPODOC}', 1, 'RVR', 0,
...
async function ensureRetencionesIvaRecibidasSetup(pool, empnit) {
  const warnings = [];
  ...
  const fmtCon = await ensureFormato(
    ...
    empnit,
    RVR_FORMATO_CON,
    'RETENCION IVA RECIBIDA AL CONTADO'
  );
  const fmtCre = await ensureFormato(
    pool,
    empnit,
    RVR_FORMATO_CRE,
    'RETENCION IVA RECIBIDA AL CREDITO'
  );

  const partCon = await ensurePartidasFormato(pool, empnit, RVR_FORMATO_CON, cuentaDebe, cuentaHaber);
  const partCre = await ensurePartidasFormato(pool, empnit, RVR_FORMATO_CRE, cuentaDebe, cuentaHaber);

  const tipoDoc = await ensureTipoDocumentoRvr(pool, empnit);

  return {
    ok: true,
    coddoc: RVR_CODDOC,
    formatos: {
      contado: RVR_FORMATO_CON,
      credito: RVR_FORMATO_CRE,
      fmtCon,
      fmtCre,
      partCon,
      partCre,
