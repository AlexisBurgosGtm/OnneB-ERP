 ...
 * Opcionalmente filtrado por cliente/proveedor (partyWhereSql + bindParty).
 */
async function fetchConsolidadoProductos(
  pool,
  sql,
  empnit,
  { tipodocSqlIn, saldoWhereSql, partyWhereSql = '', bindParty }
) {
  const request = pool.request().input('EMPNIT', sql.VarChar, empnit);
  if (typeof bindParty === 'function') bindParty(request);
...
async function fetchConsolidadoProductoDocumentos(
  pool,
  sql,
  empnit,
  codprod,
...
/**
 * Líneas de producto por documento (crédito con saldo), para imprimible de resumen.
 */
async function fetchLineasProductosPorDocumentos(
  pool,
  sql,
  empnit,
  { tipodocSqlIn, saldoWhereSql, partyWhereSql = '', bindParty }
  ...
  const result = await request.query(`
      ...
      WHERE d.EMPNIT = @EMPNIT
        ...
        AND ${saldoWhereSql}
        ${partyWhereSql || ''}
      ORDER BY d.FECHA, d.CODDOC, d.CORRELATIVO, l.Id
    `);

  const rows = (result.recordset || []).map((r) => ({
    CODDOC: String(r.CODDOC || '').trim(),
...
module.exports = {
  ...
  fetchLineasProductosPorDocumentos,
};
