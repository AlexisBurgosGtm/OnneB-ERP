...
const SALDO_APPLY = `
  OUTER APPLY (
    SELECT
      ...
      SUM(
        CAST(ISNULL(l.ENTREGADOS_TOTALUNIDADES, 0) AS FLOAT) * CAST(ISNULL(l.TIPOM, 0) AS FLOAT)
      ) AS FISICO
...
async function listInventarioRetroactivo(
  ...
  let totals = rows.reduce(
    (acc, row) => {
      ...
      acc.FISICO += Number(row.FISICO) || 0;
      acc.TOTALCOSTO += Number(row.TOTALCOSTO) || 0;
      return acc;
    },
    { SALDO: 0, FISICO: 0, TOTALCOSTO: 0 },
  );
  ...
  if (!skipExactMeta) {
    ...
    const totalsResult = await totalsReq.query(`
      SELECT
        ...
        SUM(ISNULL(inv.FISICO, 0)) AS SUM_FISICO,
...
async function listInventarioRetroactivoExport(
  ...
  return {
    ...
    totals: {
      ...
      SUM_FISICO: totalsRow.SUM_FISICO ?? 0,
