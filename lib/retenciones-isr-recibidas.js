...
} = require('./retenciones-isr-recibidas-setup');
const {
  loadCalcParams,
...
async function getTipoDocRir(pool, empnit) {
  const result = await pool
    ...
    .query(`
      ...
      WHERE EMPNIT = @EMPNIT AND CODDOC = @CODDOC AND TIPODOC = '${RIR_TIPODOC}' AND ACTIVO = 'SI'
    `);
...
async function listRetencionesIsrRecibidas(pool, empnit, mes, anio) {
  const result = await pool
    .request()
    .input('EMPNIT', sql.VarChar, empnit)
...
async function createRetencionIsrRecibida(pool, empnit, body) {
  ...
  try {
    const correlativo = await allocateCorrelativo(transaction, empnit, RIR_CODDOC);
    ...
    await transaction
      ...
      .query(`
        INSERT INTO dbo.DOCUMENTOS (
          ...
          @EMPNIT, @ANIO, @MES, @DIA, @FECHA, @HORA, @MINUTO, @CODDOC, @CORRELATIVO,
          @CODCLIENTE, @DOC_NIT, @DOC_NOMCLIE, @DOC_DIRCLIE,
          0, 0, '${CODEMBARQUE_RIR}', '${STATUS_OPERADO}', @USUARIO, 'CON', 'NO',
          'SN', @OBS, 0, 0, 'SN', 0, 1,
    ...
    return loadDocumento(pool, empnit, RIR_CODDOC, correlativo);
  } catch (err) {
...
async function updateRetencionIsrRecibida(pool, empnit, coddoc, correlativo, body) {
  const updates = [];
  const request = pool
    .request()
    .input('EMPNIT', sql.VarChar, empnit)
...
async function finalizarRetencionIsrRecibida(pool, empnit, coddoc, correlativo, body) {
  ...
  if (
    ...
    await updateRetencionIsrRecibida(pool, empnit, coddoc, correlativo, {
