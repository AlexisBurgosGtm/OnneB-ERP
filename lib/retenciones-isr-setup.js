...
const RTI_CODDOC = 'RTI';
const RTI_TIPODOC = 'RTI';
const RTI_FORMATO_CON = 'RTICON';
const RTI_FORMATO_CRE = 'RTICRE';
...
async function ensureConfigTipodoc(pool) {
  try {
    const exists = await pool.request().input('TIPODOC', sql.VarChar, RTI_TIPODOC).query(`
    ...
    await pool
      .request()
      .input('TIPODOC', sql.VarChar, RTI_TIPODOC)
      .input('DESCRIPCION', sql.VarChar, 'RETENCIONES ISR')
      .query(`
...
async function ensureTipoDocumentoRti(pool, empnit) {
  ...
  if (exists.recordset.length) {
    await pool
      ...
      .input('EMPNIT', sql.VarChar, empnit)
      .input('CODDOC', sql.VarChar, RTI_CODDOC)
      .input('CODFORMATOCON', sql.VarChar, RTI_FORMATO_CON)
      .input('CODFORMATOCRE', sql.VarChar, RTI_FORMATO_CRE)
      .query(`
  ...
  await pool
    ...
    .input('EMPNIT', sql.VarChar, empnit)
    .input('CODDOC', sql.VarChar, RTI_CODDOC)
    .input('DESDOC', sql.VarChar, 'RETENCIONES ISR')
    .input('CODFORMATOCON', sql.VarChar, RTI_FORMATO_CON)
    .input('CODFORMATOCRE', sql.VarChar, RTI_FORMATO_CRE)
    .query(`
      INSERT INTO dbo.TIPODOCUMENTOS (
        ...
        @EMPNIT, @CODDOC, @DESDOC, '${RTI_TIPODOC}', 1, 'RTI', 0,
...
async function ensureRetencionesIsrSetup(pool, empnit) {
  const warnings = [];
  ...
  const fmtCon = await ensureFormato(
    ...
    empnit,
    RTI_FORMATO_CON,
    'RETENCION ISR AL CONTADO'
  );
  const fmtCre = await ensureFormato(
    pool,
    empnit,
    RTI_FORMATO_CRE,
    'RETENCION ISR AL CREDITO'
  );

  const partCon = await ensurePartidasFormato(pool, empnit, RTI_FORMATO_CON, cuentaDebe, cuentaHaber);
  const partCre = await ensurePartidasFormato(pool, empnit, RTI_FORMATO_CRE, cuentaDebe, cuentaHaber);

  const tipoDoc = await ensureTipoDocumentoRti(pool, empnit);

  return {
    ok: true,
    coddoc: RTI_CODDOC,
    formatos: {
      contado: RTI_FORMATO_CON,
      credito: RTI_FORMATO_CRE,
      fmtCon,
      fmtCre,
      partCon,
      partCre,
